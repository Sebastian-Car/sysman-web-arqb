/*-
 * CofinanciadoresControlador.java
 *
 * 1.0
 * 
 * 05/03/2024
...
@ManagedBean
@ViewScoped
public class  CofinanciadoresControlador  extends BeanBaseContinuoAcmeImpl{
    /**
    ...
    /**
     * Crea una nueva instancia de CofinanciadoresControlador
     */
    public CofinanciadoresControlador() {
    	super();
        numFormulario = GeneralCodigoFormaEnum.COFINANCIADORES_CONTROLADOR
                        .getCodigo();
    ...
    public void inicializar() {
    	enumBase = GenericUrlEnum.COFINANCIADORES;
    	reasignarOrigen();
        buscarLlave();
    	...
    	try {
            ...
            JsfUtil.agregarMensajeError(
                            idioma.getString("MSM_TRANS_INTERRUMPIDA")
                                + ex.getMessage());
            Logger.getLogger(CofinanciadoresControlador.class.getName())
    ...
    public void cargarListacedula() {
    	UrlBean urlBean = UrlServiceUtil.getInstance().getUrlServiceByUrlByEnumID("14001");
	...
	public void seleccionarFilacedula(SelectEvent event) {
		...
		registro.getCampos().put(GeneralParameterEnum.CEDULA.getName(), 
	...
	public void seleccionarFilacedulaE(SelectEvent event) {
		Registro registroAux = (Registro) event.getObject();
	    auxiliar = registroAux.getCampos().get(GeneralParameterEnum.NIT.getName()).toString();
	    registro.getCampos().put(GeneralParameterEnum.NOMBRE.getName(), 
	    		registroAux.getCampos().get(GeneralParameterEnum.NOMBRE.getName()));
	}
    /**
