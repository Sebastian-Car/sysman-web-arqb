/*-
 * VertimientoalcantarilladoxlsControlador.java
 *
 * 1.0
 * 
 * 17/11/2016
...
import com.sysman.serviciospublicos.enums.VertimientoalcantarilladoxlsControladorEnum;
import com.sysman.serviciospublicos.enums.VertimientoalcantarilladoxlsControladorUrlEnum;
...
/**
 * Controlador del Vertimientoalcantarilladoxls
 *
 ...
 * 
 * @author eamaya
 * @version 3.0, 21/06/2017 Proceso de Refactoring y Manejo de EJBS
 * 
 */
@ManagedBean
@ViewScoped

public class VertimientoalcantarilladoxlsControlador extends BeanBaseModal {
    ...
    /**
     * Constante para el literal "8. Vert. alc.."
     */
    private static final String NOMBRE_HOJA = "8. Vert. alc.";
    // </DECLARAR_ATRIBUTOS>
    // <DECLARAR_PARAMETROS>
    // </DECLARAR_PARAMETROS>
    // <DECLARAR_LISTAS>
     ...
     */
    public VertimientoalcantarilladoxlsControlador() {
        super();
        compania = SessionUtil.getCompania();
        anoInicial = Integer.toString(GregorianCalendar.getInstance()
                        .get(GregorianCalendar.YEAR));
        anoFinal = Integer.toString(GregorianCalendar.getInstance()
                        .get(GregorianCalendar.YEAR));
        periodoInicial = "0" + Integer.toString(GregorianCalendar.getInstance()
                        .get(GregorianCalendar.MONTH)
            + 1);
        periodoFinal = "0" + Integer.toString(GregorianCalendar.getInstance()
                        .get(GregorianCalendar.MONTH)
            + 1);
        nombreArchivo = "sol_tarifas_2003_ac_alc";
        contArchivonombreExcel = new ContenedorArchivo();
        try {
            numFormulario = GeneralCodigoFormaEnum.VERTIMIENTOALCANTARILLADOXLS_CONTROLADOR
                            .getCodigo();
            ...
            listaAnoInicial = RegistroConverter.toListRegistro(
                            requestManager.getList(UrlServiceUtil.getInstance()
                                            .getUrlServiceByUrlByEnumID(
                                                            VertimientoalcantarilladoxlsControladorUrlEnum.URL8284
                                                                            .getValue())
                                            .getUrl(), param));
        }
    ...
    public void cargarListaPeriodoInicial() {
        ...
        try {
            ...
            listaPeriodoInicial = RegistroConverter.toListRegistro(
                            requestManager.getList(UrlServiceUtil.getInstance()
                                            .getUrlServiceByUrlByEnumID(
                                                            VertimientoalcantarilladoxlsControladorUrlEnum.URL8727
                                                                            .getValue())
                                            .getUrl(), param));
        }
        catch (SystemException e) {
    ...
    public void cargarListaAnoFinal() {
        ...
        try {
            ...
            listaAnoFinal = RegistroConverter.toListRegistro(
                            requestManager.getList(UrlServiceUtil.getInstance()
                                            .getUrlServiceByUrlByEnumID(
                                                            VertimientoalcantarilladoxlsControladorUrlEnum.URL9240
                                                                            .getValue())
                                            .getUrl(), param));
        }
        catch (SystemException e) {
    ...
    public void cargarListaPeriodoFinal() {
        ...
        try {
            ...
            listaPeriodoFinal = RegistroConverter.toListRegistro(
                            requestManager.getList(UrlServiceUtil.getInstance()
                                            .getUrlServiceByUrlByEnumID(
                                                            VertimientoalcantarilladoxlsControladorUrlEnum.URL9757
                                                                            .getValue())
                                            .getUrl(), param));
        }
        catch (SystemException e) {
    ...
    public void cargarListaCiclo() {
        ...
        try {
            listaCiclo = RegistroConverter.toListRegistro(
                            requestManager.getList(UrlServiceUtil.getInstance()
                                            .getUrlServiceByUrlByEnumID(
                                                            VertimientoalcantarilladoxlsControladorUrlEnum.URL10341
                                                                            .getValue())
                                            .getUrl(), null));
        }
        catch (SystemException e) {
    ...
    public void cargarListaNombreHoja() {
        ...
        try {
            listaNombreHoja = RegistroConverter.toListRegistro(
                            requestManager.getList(UrlServiceUtil.getInstance()
                                            .getUrlServiceByUrlByEnumID(
                                                            VertimientoalcantarilladoxlsControladorUrlEnum.URL10984
                                                                            .getValue())
                                            .getUrl(), null));
        }
        catch (SystemException e) {
    ...
    public void oprimirExcel() {
        ...
        try {
            ...
            param.put(VertimientoalcantarilladoxlsControladorEnum.ANOINICIAL
                            .getValue(), anoInicial);

            param.put(VertimientoalcantarilladoxlsControladorEnum.PERIODOINICIAL
                            .getValue(), periodoInicial);

            param.put(VertimientoalcantarilladoxlsControladorEnum.ANOFINAL
                            .getValue(), anoFinal);

            param.put(VertimientoalcantarilladoxlsControladorEnum.PERIODOFINAL
                            .getValue(), periodoFinal);

            listaPivot = RegistroConverter.toListRegistro(
                            requestManager.getList(UrlServiceUtil.getInstance()
                                            .getUrlServiceByUrlByEnumID(
                                                            VertimientoalcantarilladoxlsControladorUrlEnum.URL13710
                                                                            .getValue())
                                            .getUrl(), param));
    ...
    private void generarExcel() {
        ...
        try (FileInputStream fileIs = new FileInputStream(
            ...
            StringBuilder aux = new StringBuilder();
            for (Registro registro : listaPivot) {
                aux.append("'" + registro.getCampos().get(PERIODO).toString()
                    + "' AS \""
                    + registro.getCampos().get(PERIODO).toString() + "\",");
            }
            inPivot = aux.toString();
            if (inPivot.length() > 0) {
                inPivot = inPivot.substring(0, inPivot.length() - 1);
            ...
            String strSql = Reporteador.resuelveConsulta(
                            "800071InformeVertimientoAlcantarilladorXls",
                            Integer.parseInt(SessionUtil.getModulo()),
                            reemplazos);
            ...
            for (Registro registro : rs) {
                Row row = sheet.createRow(registro.getIndice() + 10);
                index = 1;
                for (Registro reg : listaPivot) {
                    Cell cell = row.createCell(index);
                    Object value = registro.getCampos()
                                    .get(reg.getCampos().get(PERIODO)
                                                    .toString()) != null
                                                        ? registro.getCampos()
                                                                        .get(reg.getCampos()
                                                                                        .get(PERIODO)
                ...
                CellReference cellRefIni = new CellReference(
                                registro.getIndice() + 10, 1);
                CellReference cellRefFin = new CellReference(
                                registro.getIndice() + 10, index - 1);
                ...
                cellTotal.setCellStyle(styleRojo);
            }
            ...
            cell3.setCellValue(idioma.getString("TB_TB2249") + " "
    ...
    private void cargarNombreHojas(Workbook workbook) {
        ...
        for (int i = 0; i < hojas; i++) {
            ...
            reg.getCampos().put("X", hoja);
            listaNombreHoja.add(reg);
        }
    }

    /**
     * Retorna la variable obtenerNombreMes
     * 
     * @return res cambia el numero por el nombre
     */
