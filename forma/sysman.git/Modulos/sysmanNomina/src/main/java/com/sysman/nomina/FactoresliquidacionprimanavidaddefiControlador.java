/*-
 * FactoresliquidacionprimanavidaddefiControlador.java
 *
 * 1.0
 * 
 * 27/11/2020
...
import com.sysman.nomina.enums.factoresliquidacionprimanavidaddeficontroladorUrlEnum;
...
@ManagedBean
@ViewScoped
public class  FactoresliquidacionprimanavidaddefiControlador extends BeanBaseModal{
	/**
	...
	//</DECLARAR_LISTAS_COMBO_GRANDE>
	/**
	 * Crea una nueva instancia de FactoresliquidacionprimanavidaddefiControlador
	 */
	@EJB
	 ...
	 */
	public FactoresliquidacionprimanavidaddefiControlador() {
		super();
		...
		try {
			//2212
			numFormulario = GeneralCodigoFormaEnum.FACTORES_LIQUIDACION_PRIMANAVIDADDEFI_CONTROLADOR.getCodigo();
			validarPermisos();
		} catch (Exception ex) {
			...
			listaAno1 = RegistroConverter
					.toListRegistro(requestManager.getList(
							...
							.getUrlServiceByUrlByEnumID(
									factoresliquidacionprimanavidaddeficontroladorUrlEnum.URL4061.getValue())	
							.getUrl(),
							param));
		...
		try {
			listaMes1 = RegistroConverter
					.toListRegistro(requestManager.getList(
							UrlServiceUtil.getInstance()
							.getUrlServiceByUrlByEnumID(
									factoresliquidacionprimanavidaddeficontroladorUrlEnum.URL4062.getValue())
							.getUrl(),
							param));
		} catch (SystemException e) {
			logger.error(e.getMessage(), e);
		...
		try {
			listaPeriodo1 = RegistroConverter
					.toListRegistro(requestManager.getList(
							UrlServiceUtil.getInstance()
							.getUrlServiceByUrlByEnumID(
									factoresliquidacionprimanavidaddeficontroladorUrlEnum.URL4063.getValue())
							.getUrl(),
							param));
		} catch (SystemException e) {
			logger.error(e.getMessage(), e);
	...
	public void generarInforme(ReportesBean.FORMATOS formato){
		try {
			...
			Reporteador.resuelveConsulta("002134FACTORESLIQUIDACIONPRIMANAVIDADIDI",
			...
			archivoDescarga = JsfUtil.exportarStreamed(
					"002134FACTORESLIQUIDACIONPRIMANAVIDADIDI", parametros,
