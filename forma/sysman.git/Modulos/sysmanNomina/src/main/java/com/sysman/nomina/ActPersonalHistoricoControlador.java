/*-
 * ActPersonalHistoricoControlador.java
 *
 * 1.0
 * 
 * 09/01/2024
...
import com.sysman.nomina.enums.ActPersonalHistoricoControladorUrlEnum;
/**
 *
...
@ManagedBean
@ViewScoped
public class  ActPersonalHistoricoControlador extends BeanBaseModal{
	/**
	...
	/**
	 * Crea una nueva instancia de ActPersonalHistoricoControlador
	 */
	public ActPersonalHistoricoControlador() {
		super();
		compania = SessionUtil.getCompania();
		try {
			//2447
			numFormulario = GeneralCodigoFormaEnum.ACT_PERSONAL_HISTORICO_CONTROLADOR.getCodigo();
			validarPermisos();
					...
					.toListRegistro(
							requestManager.getList(
									...
									.getUrlServiceByUrlByEnumID(
											ActPersonalHistoricoControladorUrlEnum.URL7086.getValue())
									.getUrl(),
									param));
			...
			listaAnio = RegistroConverter
					.toListRegistro(
							requestManager.getList(
									UrlServiceUtil.getInstance()
									.getUrlServiceByUrlByEnumID(
											ActPersonalHistoricoControladorUrlEnum.URL5665.getValue())
									.getUrl(),
									param));
		} catch (SystemException e) {
			JsfUtil.agregarMensajeError(e.getMessage());
			...
			listaMes = RegistroConverter
					.toListRegistro(
							requestManager.getList(
									UrlServiceUtil.getInstance()
									.getUrlServiceByUrlByEnumID(
											ActPersonalHistoricoControladorUrlEnum.URL6127.getValue())
									.getUrl(),
									param));
		} catch (SystemException e) {
			JsfUtil.agregarMensajeError(e.getMessage());
			...
			listaPeriodo = RegistroConverter
					.toListRegistro(
							requestManager.getList(
									UrlServiceUtil.getInstance()
									.getUrlServiceByUrlByEnumID(
											ActPersonalHistoricoControladorUrlEnum.URL6665.getValue())
									.getUrl(),
									param));
		} catch (SystemException e) {
			JsfUtil.agregarMensajeError(e.getMessage());
	...
	public void oprimirEjecutar() {
		...
		try {
			...
			nominaDiezRemote.actPersonalHist(compania, Integer.parseInt(proceso), Integer.parseInt(anio), Integer.parseInt(mes), Integer.parseInt(periodo), SessionUtil.getUser().getCodigo());
