...
public class DiscoBancolombiaControlador extends BeanBaseModal {
	/**
	 ...
	 */
	public DiscoBancolombiaControlador() {
		super();
	...
	public void oprimirbtEnviarExcel() {
		...
		try {
			...
			String datos = ejbNominaCuatro.generarDiscoBancoColombia(compania, Integer.parseInt(proceso),
					Integer.parseInt(anio), Integer.parseInt(mes), Integer.parseInt(periodo),
					SysmanFunciones.nvlStr(banco, " "), fechaReporte, todosLosBancos, observaciones, lote, 2,
					tipoCuentaBanOrigen, cuentaBancoOrigen);
			generarExcel(datos);
	...
	public void oprimirbtGenerarPlano() {
		...
		try {
			...
			logger.error(e.getMessage(), e);
			JsfUtil.agregarMensajeError("Asegurese de configurar los parametros ARCHIVO PLANO BANCOLOMBIA TXT y FORMATO ESPECIAL BANCOS "+e.getMessage());
		}
