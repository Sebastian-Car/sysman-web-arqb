/*-
 * BalanceGeneralNivelesControlador.java
 *
 * 1.0
 * 
 * 04/02/2019
...
import com.sysman.contabilidad.enums.BalanceGeneralNivelesControladorUrlEnum;
...
/**
 * Formulario que permite generar el informe de Balance General por
 * Niveles.
 *
...
@ManagedBean
@ViewScoped
public class BalanceGeneralNivelesControlador extends BeanBaseModal {
    /**
    ...
    /**
     * Crea una nueva instancia de BalanceGeneralNivelesControlador
     */
    public BalanceGeneralNivelesControlador() {
        super();
        compania = SessionUtil.getCompania();
        modulo = SessionUtil.getModulo();
        try {
            numFormulario = GeneralCodigoFormaEnum.BALANCE_GENERALNIVELES_CONTROLADOR
                            .getCodigo();
            validarPermisos();
            // <INI_ADICIONAL>
            // </INI_ADICIONAL>
    ...
    public void abrirFormulario() {
        // <CODIGO_DESARROLLADO>
        if ("102010102".equals(SessionUtil.getMenuActual())) {
            titulo = idioma.getString("TB_TB4279");
        }
        else {
            titulo = idioma.getString("TB_TB4280");
        }
        // </CODIGO_DESARROLLADO>
    }
            ...
            listaAnio = RegistroConverter.toListRegistro(
                            requestManager.getList(UrlServiceUtil.getInstance()
                                            .getUrlServiceByUrlByEnumID(
                                                            BalanceGeneralNivelesControladorUrlEnum.URL3837
                                                                            .getValue())
                                            .getUrl(), param));
        }
    ...
    public void cargarListaCodigoInicial() {
        ...
        if("2".equals(tipoInforme)) {
            urlBean = UrlServiceUtil.getInstance()
                            .getUrlServiceByUrlByEnumID(
                                            BalanceGeneralNivelesControladorUrlEnum.URL5417
                                                            .getValue());
        }else {
            urlBean = UrlServiceUtil.getInstance()
                            .getUrlServiceByUrlByEnumID(
                                            BalanceGeneralNivelesControladorUrlEnum.URL4143
                                                            .getValue());
        }
    ...
    public void cargarListaCodigoFinal() {
        UrlBean urlBean = UrlServiceUtil.getInstance()
                        .getUrlServiceByUrlByEnumID(
                                        BalanceGeneralNivelesControladorUrlEnum.URL4737
                                                        .getValue());
        Map<String, Object> param = new TreeMap<>();
        param.put(GeneralParameterEnum.COMPANIA.getName(), compania);
        param.put(GeneralParameterEnum.ANO.getName(), anio);
    ...
    public void generarInforme(FORMATOS formato) {
        ...
        try {
            ...
            String informe;
            if ("102010102".equals(SessionUtil.getMenuActual())) {
                informe = "001988BalanceGeneralNiveles";
                parametros.put("PR_NIVEL", "1");
            }
            else {
                informe = "001990BalanceGeneralACuatroNiveles";
                parametros.put("PR_NIVEL", "2");
            }
            ...
            if ("1".equals(tipoInforme)) {
                tituloInf = "BALANCE GENERAL A ";
            }
            else {
                tituloInf = "ESTADO DE RESULTADOS A ";
            }

            Calendar calendario = Calendar.getInstance();
            calendario.set(anio, mes, 0);
            int ultimoDiaMes = calendario
                            .getActualMaximum(Calendar.DAY_OF_MONTH);
            String fecha = (ultimoDiaMes + "/" + (mes < 10 ? "0" + mes : mes)
                + "/" + anio);

            if (mes == 0) {
                tituloInf += "MES INICIAL DE " + anio;
            }
            else if (mes > 0 && mes < 13) {
                tituloInf += fecha;
            }
            else {
                tituloInf += "MES FINAL DE " + anio;
            }

            parametros.put("PR_TIPODEINFORME", tituloInf);
            parametros.put("PR_TIPO", tipoInforme);

            parametros.put("PR_NOMBRECOMPANIA",
                            SessionUtil.getCompaniaIngreso().getNombre());

            parametros.put("PR_CODIGOINI",
                            codigoInicial);

            parametros.put("PR_CODIGOFIN",
                            codigoFinal);

            archivoDescarga = JsfUtil.exportarStreamed(
                            "001988BalanceGeneralNiveles", parametros,
    ...
    public void cambiarTipoDeinforme()
    {
        cargarListaCodigoInicial();
