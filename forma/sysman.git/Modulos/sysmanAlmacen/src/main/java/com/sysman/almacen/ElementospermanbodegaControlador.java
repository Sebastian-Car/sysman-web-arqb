/*-
 * ElementospermanbodegaControlador.java
 *
 * 1.0
 * 
 * 24/10/2018
...
import com.sysman.almacen.enums.ElementospermanbodegaControladorUrlEnum;
...
/**
 * Formulario que permite imprimir reporte de los elementos que permanecen en bodega
 *
...
@ManagedBean
@ViewScoped
public class  ElementospermanbodegaControlador extends BeanBaseModal{
	/**
	...
	public ElementospermanbodegaControlador() {
		super();
		compania = SessionUtil.getCompania();
		modulo = SessionUtil.getModulo();
		try {
			numFormulario = GeneralCodigoFormaEnum.ELEMENTOS_PERMAN_BODEGA_CONTROLADOR.getCodigo();
			validarPermisos();
			//<INI_ADICIONAL>
			//</INI_ADICIONAL>
		} catch (Exception ex) {
	...
	public void cargarListacmbElementoDesde(){
		UrlBean urlBean = UrlServiceUtil.getInstance()
				.getUrlServiceByUrlByEnumID(ElementospermanbodegaControladorUrlEnum.URL001.getValue());
	...
	public void cargarListacmbElementoHasta(){
		UrlBean urlBean = UrlServiceUtil.getInstance()
				.getUrlServiceByUrlByEnumID(ElementospermanbodegaControladorUrlEnum.URL002.getValue());
		Map<String, Object> param = new TreeMap<>();
		param.put(GeneralParameterEnum.COMPANIA.getName(), compania);
	...
	{
		try{
			reporte= "001946ELEMENTOSPERMABODEGA";
			...
			parametros.put("PR_BODEGA_FECHAINICIAL", SysmanFunciones.convertirAFechaCadena(fechaInicial));
