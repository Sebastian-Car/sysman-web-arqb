/*-
 * KardexConsumoBodegaControlador.java
 *
 * 1.0
 * 
 * 13/05/2025
...
@ManagedBean
@ViewScoped
public class KardexConsumoBodegaControlador extends BeanBaseModal {
	/**
	...
	public KardexConsumoBodegaControlador() {
		super();
		...
		try {
			numFormulario = GeneralCodigoFormaEnum.KARDEX_COSUMO_CONT_CONTROLADOR.getCodigo();
			validarPermisos();
		} catch (Exception ex) {
	...
	public void abrirFormulario() {
		if ("1004020211".equals(SessionUtil.getMenuActual())) {
			titulo = idioma.getString("TB_TB4476");
			reporte = "002785KardexConsumoCont";
			consulta = reporte;
		} else if ("1004020212".equals(SessionUtil.getMenuActual())) {
			titulo = idioma.getString("TB_TB4479");
			reporte = "002785KardexConsumoCont";
			consulta = "8000576KardexConsumoCont";
		} else {
			titulo = "";
		}
	}
	...
	public void cargarListaCmbElementoDesde() {
		UrlBean urlBean = UrlServiceUtil.getInstance()
				.getUrlServiceByUrlByEnumID(KardexControladorUrlEnum.URL11488.getValue());
...
//<METODOS_BOTONES>
	...
	private void presentarInforme(FORMATOS formatos) {
		try {
			...
			String parametro = ejbSysmanUtil.consultarParametro(compania, "DIGITOS AGRUPACION INVENTARIO", modulo,
					new Date(), true);
			...
			parametros.put(cStrsql, Reporteador.resuelveConsulta(consulta, Integer.parseInt(modulo), remplazar));
...
//<SET_GET_ATRIBUTOS>
	 ...
	 * 
	 * @param elementoDesde Variable a asignar en elementoDesde
	 */
	 ...
	 * 
	 * @param elementoHasta Variable a asignar en elementoHasta
	 */
	 ...
	 * 
	 * @param listaCmbElementoDesde Variable a asignar en listaCmbElementoDesde
	 */
	 ...
	 * 
	 * @param listaCmbElementoHasta Variable a asignar en listaCmbElementoHasta
	 */
