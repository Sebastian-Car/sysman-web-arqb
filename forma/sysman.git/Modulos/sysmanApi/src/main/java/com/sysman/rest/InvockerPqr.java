...
public class InvockerPqr {
    ...
    public Response crearSolicitudAutoservicio(Pqr contexto) {
        ...
        try {
		contexto.setTipoConsumo("1");
                procesador.setContexto(contexto);
                procesador.procesar();
                respuestaApi.setCuerpo(procesador.getResultado().getCuerpo());
        }
    ...
    public Response consultarPqr(
        @QueryParam("numRadicado") BigInteger numRadicado,
        @QueryParam("cedula") String cedula,
        @QueryParam("compania") String compania,
        @QueryParam("codigoTramite") String codigoTramite
       ) {
       RespuestaApi respuestaApi = new RespuestaApi();
        idioma = ResourceBundle.getBundle(SysmanConstantes.RUTA_IDIOMA);
       try { 
    	   paramPqr.setNumRadicado(numRadicado);
    	   paramPqr.setTipoConsumo("0");
    	   paramPqr.setCodigoTramite(codigoTramite);
    	   paramPqr.setCompania(compania);
    	   paramPqr.setCedula(cedula);
	   	   procesador.setContexto(paramPqr);
           procesador.procesar();
           respuestaApi.setCuerpo(procesador.getResultado().getCuerpo());
