/*-
 * ProgramacionSectorControlador.java
 *
 * 1.0
 * 
 * 19/11/2018
...
import com.sysman.plandesarrollo.enums.ProgramacionSectorControladorUrlEnum;
...
/**
 * Clase migrada para generar el informe de POAI de programacion de
 * sectores
 *
 * @version 1.0, 19/11/2018
 * @author ybecerra
 * 
 * @version 1.5, 07/05/2019
 * @author jrodrigueza
 */
@ManagedBean
@ViewScoped
public class ProgramacionSectorControlador extends BeanBaseModal {
    /**
    ...
    // </DECLARAR_LISTAS_COMBO_GRANDE>
    /**
     * Crea una nueva instancia de ProgramacionSectorControlador
     */
    public ProgramacionSectorControlador() {
        super();
        compania = SessionUtil.getCompania();
        try {
            // 1989
            numFormulario = GeneralCodigoFormaEnum.PROGRAMACION_POR_SECTOR_CONTROLADOR
                            .getCodigo();
            validarPermisos();
            // <INI_ADICIONAL>
            // </INI_ADICIONAL>
            ...
            listaVigencia = RegistroConverter.toListRegistro(
                            requestManager.getList(UrlServiceUtil.getInstance()
                                            .getUrlServiceByUrlByEnumID(
                                                            ProgramacionSectorControladorUrlEnum.URL126
                                                                            .getValue())
                                            .getUrl(), param));
        }
    ...
    public void generarExcel() {
        try {
            String codigoTipo = "58";
            String codigoPlantilla = "67003";
            Map<String, Object> param = new TreeMap<>();
            param.put(GeneralParameterEnum.COMPANIA.getName(), compania);
            param.put(GeneralParameterEnum.TIPO.getName(), codigoTipo);
            param.put(GeneralParameterEnum.CODIGO.getName(), codigoPlantilla);
            ...
            Date date = null;
            if (rsFecha != null) {
                Map<String, Object> campos = rsFecha.getCampos();
                date = (Date) campos.get(GeneralParameterEnum.FECHA.getName());
            }
            else {
                String msg = idioma.getString("TG_CONFIGURE_PLANTILLA");
                msg = msg.replace("s$codigoPlantilla$s", codigoPlantilla);
                msg = msg.replace("s$codigoTipo$s", codigoTipo);
                JsfUtil.agregarMensajeAlertaDialogo(msg);
                return;
            }
            String fechaPlantilla = SysmanFunciones.formatearFecha(date);
            String nombreDocDescarga = idioma.getString("TB_TB4253");
            archivoDescarga = UtilitarioPlantillas.exportarDocumento(
                            codigoPlantilla, fechaPlantilla, variablesConsulta,
                            nombreDocDescarga, null);
        }
        catch (SystemException e) {
            logger.error(e.getMessage(), e);
            JsfUtil.agregarMensajeError(e.getMessage());
