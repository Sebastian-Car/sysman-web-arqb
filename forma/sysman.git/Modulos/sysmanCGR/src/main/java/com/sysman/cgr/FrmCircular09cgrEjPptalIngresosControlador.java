...
import com.sysman.cgr.enums.FrmCircular09cgrEjPptalIngresosControladorEnum;
import com.sysman.cgr.enums.FrmCircular09cgrEjPptalIngresosControladorUrlEnum;
...
@ManagedBean
@ViewScoped
public class  FrmCircular09cgrEjPptalIngresosControlador extends BeanBaseModal{
    /**
...
//<DECLARAR_ATRIBUTOS>
     ...
     */
	public FrmCircular09cgrEjPptalIngresosControlador() {
	    super();
	    compania = SessionUtil.getCompania();
	    nitCompania = SessionUtil.getCompaniaIngreso().getNit();
	    nomCompania = SessionUtil.getCompaniaIngreso().getNombre();
	    try {
	    	numFormulario=GeneralCodigoFormaEnum.FRMCIRCULAR09CGREJPPTALINGRESOS
	              .getCodigo();
	            validarPermisos();
        ...
        listavigencia = RegistroConverter.toListRegistro(
                        requestManager.getList(UrlServiceUtil.getInstance()
                                        .getUrlServiceByUrlByEnumID(
                                        		FrmCircular09cgrEjPptalIngresosControladorUrlEnum.URL0001
                                                                        .getValue())
                                        .getUrl(), param));
    }
...
public void cargarListames(){
	...
	try {
        ...
        listames = RegistroConverter.toListRegistro(
                        requestManager.getList(UrlServiceUtil.getInstance()
                                        .getUrlServiceByUrlByEnumID(
                                        		FrmCircular09cgrEjPptalIngresosControladorUrlEnum.URL0002
                                                                        .getValue())
                                        .getUrl(), param));
    }
    catch (SystemException e) {
...
public void cargarListacodInicial(){
	...
	UrlBean urlBean = UrlServiceUtil.getInstance()
            .getUrlServiceByUrlByEnumID(
            		FrmCircular09cgrEjPptalIngresosControladorUrlEnum.URL0003
                                            .getValue());
	Map<String, Object> param = new TreeMap<>();
...
public void cargarListacodFinal(){
	UrlBean urlBean = UrlServiceUtil.getInstance()
            .getUrlServiceByUrlByEnumID(
            		FrmCircular09cgrEjPptalIngresosControladorUrlEnum.URL0004
                                            .getValue());
	Map<String, Object> param = new TreeMap<>();
	param.put(GeneralParameterEnum.COMPANIA.getName(),
	            compania);
	param.put(FrmCircular09cgrEjPptalIngresosControladorEnum.PARAM2.getValue(),
	            codInicial);
	param.put(GeneralParameterEnum.ANO.getName(),
	            anio);
...
private String resolverConsulta() {
    ...
    return Reporteador.resuelveConsulta("800436circular09EjPptalIngresos",
                    modulo, reemplazos);
}
...
public void generarArchivo(String content, String extension) {
    ...
    try {
        ...
        if (".csv".equals(extension)) {
            nombreArchivo = idioma.getString("TT_FR977") + extension;
            ...
            textoArchivo.append(idioma.getString("TB_TB4368")
                    + ",");
            textoArchivo.append(idioma.getString("TB_TB4370")
            	+ ",");
            textoArchivo.append(idioma.getString("TB_TB4371").toUpperCase()
            	+ ",");
            textoArchivo.append(idioma.getString("TG_APROPIACION_INICIAL2")
            	+ ",");
            textoArchivo.append(idioma.getString("TG_ADICIONES")
            	+ ",");
            textoArchivo.append(idioma.getString("TG_PRESUPUESTO_DEFINITIVO")
            	+ ",");
            textoArchivo.append(idioma.getString("TB_TB4372")
            	+ ",");
            textoArchivo.append(idioma.getString("TB_TB4366")
            	+ ",");
            textoArchivo.append(idioma.getString("TG_AJUSTES").toUpperCase()            		
            	+ ",");
            textoArchivo.append(idioma.getString("TG_RECONOCIMIENTO").toUpperCase()            		
            	+ ",");
            textoArchivo.append(idioma.getString("TB_TB4373")
                	+ ",");
            textoArchivo.append(idioma.getString("TB_TB4374")
                	+ "\n");
            for (Registro registro : listado) {
                textoArchivo.append(SysmanFunciones
                                .nvl(registro.getCampos().get("VIGENCIA"), "")
                    + ",");
                textoArchivo.append(SysmanFunciones
            ...
            + ",");
                textoArchivo.append(SysmanFunciones
                        .nvl(registro.getCampos()
                                        .get("RECONOCIMIENTOS"), 0)                                                         
                        .toString().replace(",", "")
            + ",");
                textoArchivo.append(SysmanFunciones
                        .nvl(registro.getCampos()
                                        .get("RECAUDOSTOTALES"), 0)
                        .toString().replace(",", "")
            + ",");
                textoArchivo.append(SysmanFunciones
                        .nvl(registro.getCampos()
                                        .get("PORCENTAJE_EJECUCION"), 0)
                        .toString().replace(",", "")
            + "\n");
            }
        }
