MERGE INTO CONSULTAS FIN USING (SELECT '002612AuxCuentaCCostoReferencia' INFORME ,TO_CLOB(q'[SELECT  NOCOMP,
        NOANO,
        ID AS CUENTA,
        NOMBRECODIGO AS PNOMBRE,
        CUENTANOMB,
        TRIM(RPAD(ID,32)) AS CUENTACOD,
        VD AS VALOR_DEBITO,
        VC AS VALOR_CREDITO,
...
SELECT 
       ...
       END TERCERO
FROM PLAN_CONTABLE P INNE]') || TO_CLOB(q'[R JOIN   SALDO_AUX_CONTABLE SA 
...
,
                                 ...
                                 DETALLE_COMPROBANTE_CNT.FECHA,                                 
                                  CASE WHEN DETALLE_COMPROBANTE_CNT.FECHA>= s$fechaInicial$s THE]') || TO_CLOB(q'[N
            ...
            AND DETALLE_COMPROBANTE_CNT.REFERENCIA BETWEEN 's$refiInicial$s' AND  's$refFinal$s'
                     ...
                     ]') || TO_CLOB(q'[    AND SA.AUXILIAR       = DE.AUXILIARI
...
GROUP BY NOCOMP, NOANO, ID, VD, VC,CUENTANOMB, TRIM(RPAD(ID,32)), 
TIPO_CPTE, COMPROBANTE, CONSECUTIVO, FECHA, HORA, 
DESCRIPCION, NODOCUMENTO, SALDOINI, DESCRIP, NOMBRETERCERO, 
NOMBRECODIGO, CODIGOREF, TERCERO, ORDENIMP, 
CENTRO_COSTO, NOMBRECENTROCOSTO]') CONSULTA, 1 APLICACION ,TO_CLOB(q'[]') CONSULTA_OPCIONAL, NULL CREATED_BY, NULL MODIFIED_BY  FROM DUAL ) INI ON (INI.INFORME = FIN.INFORME )  WHEN MATCHED THEN  UPDATE SET FIN.CONSULTA =  INI.CONSULTA, FIN.APLICACION =  INI.APLICACION, FIN.CONSULTA_OPCIONAL =  INI.CONSULTA_OPCIONAL, FIN.MODIFIED_BY = INI.MODIFIED_BY, FIN.DATE_MODIFIED = SYSDATE  WHEN NOT MATCHED THEN  INSERT (INFORME,CONSULTA, APLICACION,CONSULTA_OPCIONAL,CREATED_BY,DATE_CREATED)  VALUES (INI.INFORME,INI.CONSULTA, INI.APLICACION,INI.CONSULTA_OPCIONAL,INI.CREATED_BY,SYSDATE);