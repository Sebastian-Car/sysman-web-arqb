MERGE INTO CONSULTAS FIN USING (SELECT '8000574LisAuxiliarSaldos' INFORME ,TO_CLOB(q'[SELECT  
       ANO,
       ID,
       TIPO_CPTE,
       COMPROBANTE,
...
SELECT
       ...
       NVL(DE.DESCRIPCION_DET,DE.DESCRIPCION_CNT) DESCRIPCION,
...
FROM PLAN_CONTABLE P INNER JOIN   SALDO_AUX_CONTABLE SA
  ...
  AND P.CODIGO  = SA.CODIGO
   LEFT JOIN ( SELECT COMPANIA,ANO,CUENTA, SUM( CASE WHEN  TO_DATE('s$fechaInicial$s', 'DD/MM/YYYY')=  TO_DATE('01'||'/'||TO_CHAR(TO_DATE('s$fechaInicial$s','DD/MM/YYYY'),'MM')||'/'||TO_CHAR(TO_DATE('s$fechaInicial$s','DD/MM/YYYY'),'YYYY'), 'DD/MM/YYYY' )THEN 0 ELSE  CASE WHEN DETALLE_COMPROBANTE_CNT.naturaleza = 'D' THEN DETALLE_COMPROBANTE_CNT.valor_debito - DETALLE_COMPRO]') || TO_CLOB(q'[BANTE_CNT.valor_credito ELSE DETALLE_COMPROBANTE_CNT.valor_credito - DETALLE_COMPROBANTE_CNT.valor_debito END END )
                                        SALDOALDIA
...
,
                                 ...
                                 END VALOR_DEBITO,
                                 CASE WHEN DETALLE_COMPROBANTE_CNT.FECHA>= TO_DATE('s$fechaInicial$s', 'DD/MM/YYYY' ) THEN
                             ]') || TO_CLOB(q'[               DETALLE_COMPROBANTE_CNT.VALOR_CREDITO
                                 ELSE
                                        0
                                 END VALOR_CREDITO,
 ...
 INNER JOIN TERCERO TERCERO_SA
   ...
   AND COALESCE(DE.TERCERO, SA.TERC]') || TO_CLOB(q'[ERO) = TERCERO_SA.NIT
...
GROUP BY 
       ...
       DESCRIPCION,
       NOMBRETERCERO,
       TERCERO]') CONSULTA, 1 APLICACION ,TO_CLOB(q'[]') CONSULTA_OPCIONAL, NULL CREATED_BY, NULL MODIFIED_BY  FROM DUAL ) INI ON (INI.INFORME = FIN.INFORME )  WHEN MATCHED THEN  UPDATE SET FIN.CONSULTA =  INI.CONSULTA, FIN.APLICACION =  INI.APLICACION, FIN.CONSULTA_OPCIONAL =  INI.CONSULTA_OPCIONAL, FIN.MODIFIED_BY = INI.MODIFIED_BY, FIN.DATE_MODIFIED = SYSDATE  WHEN NOT MATCHED THEN  INSERT (INFORME,CONSULTA, APLICACION,CONSULTA_OPCIONAL,CREATED_BY,DATE_CREATED)  VALUES (INI.INFORME,INI.CONSULTA, INI.APLICACION,INI.CONSULTA_OPCIONAL,INI.CREATED_BY,SYSDATE);
COMMIT;