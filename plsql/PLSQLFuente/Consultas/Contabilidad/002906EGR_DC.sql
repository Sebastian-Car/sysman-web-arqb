MERGE INTO CONSULTAS FIN USING ( SELECT '002906EGR_DC' AS INFORME_VAL, TO_CLOB(q'[SELECT COMPROBANTE_CNT.TIPO,
PLAN_CONTABLE.CODIGO,
    ...
    CASE WHEN NVL(TERCERO.NIT,'') = '' THEN '' ELSE '/' || NVL(TERCERO.NOMBRE,'') END ||
...
AND DETALLE_COMPROBANTE_CNT.COMPROBA]') || TO_CLOB(q'[NTE = COMPROBANTE_CNT.NUMERO
LEFT JOIN TERCERO
...
]') AS CONSULTA_VAL, 1 AS APLICACION_VAL, TO_CLOB(q'[]') AS CONSULTA_OPCIONAL_VAL, (SELECT MIN(ROWID) FROM CONSULTAS WHERE INFORME = '002906EGR_DC') AS RID_DESTINO  FROM DUAL ) INI ON (FIN.ROWID = INI.RID_DESTINO)  WHEN MATCHED THEN   UPDATE SET FIN.CONSULTA = INI.CONSULTA_VAL, FIN.APLICACION = INI.APLICACION_VAL, FIN.CONSULTA_OPCIONAL = INI.CONSULTA_OPCIONAL_VAL, FIN.DATE_MODIFIED = SYSDATE  WHEN NOT MATCHED THEN   INSERT (INFORME, CONSULTA, APLICACION, CONSULTA_OPCIONAL, DATE_CREATED)   VALUES (INI.INFORME_VAL, INI.CONSULTA_VAL, INI.APLICACION_VAL, INI.CONSULTA_OPCIONAL_VAL, SYSDATE);
