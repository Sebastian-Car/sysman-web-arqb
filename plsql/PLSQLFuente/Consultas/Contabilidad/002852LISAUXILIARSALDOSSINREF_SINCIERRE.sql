MERGE INTO CONSULTAS FIN USING (SELECT '002852LISAUXILIARSALDOSSINREF_SINCIERRE' INFORME ,TO_CLOB(q'[select *
from (
SELECT 
       P.COMPANIA NOCOMP,
       P.ANO NOANO, 
  ...
  AND P.CODIGO  = SA.CODIGO
                LEFT JOIN 
                         (SELECT DETALLE_COMPROBANTE_CNT.COMPANIA, 
                                 ...
                                 DET]') || TO_CLOB(q'[ALLE_COMPROBANTE_CNT.FUENTE_RECURSOI,
                    ...
                    WHERE CNT.COMPANIA= s$compania$s
                      ...
                      AND CNT]') || TO_CLOB(q'[.FECHA BETWEEN TO_DATE('01/' || TO_CHAR(TO_DATE('s$fechaInicial$s', 'DD/MM/YYYY'),'MM/YYYY')) AND TO_DATE('s$fechaFinal$s', 'DD/MM/YYYY' )
