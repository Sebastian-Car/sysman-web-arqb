MERGE INTO CONSULTAS FIN USING (SELECT '002623AuxTerceroCuentaCostoRefporCuenta' INFORME ,TO_CLOB(q'[SELECT  NOCOMP,
        NOANO,
...
FROM PLAN_CONTABLE P INNER JOIN   SALDO_AUX_CONTABLE SA
  ...
  AND P.CODIGO  = SA.CODIGO
   LEFT JOIN ( SELECT COMPANIA,ANO,CUENTA,TERCERO, SUM( CASE WHEN  s$fechaInicial$s =  TO_DATE('01'||'/'||TO_CHAR(s$fechaInicial$s,'MM')||'/'||TO_CHAR(s$fechaInicial$s,'YYYY'), 'DD/MM/YYYY' )THEN 0 ELSE  CASE WHEN DETALLE_COMPROBANTE_CNT.naturaleza = 'D' THEN DETALLE_COMPROBANTE_CNT.valor_debito - DETALLE_COMPROBANTE_CNT.valor_credito ELSE DETALLE_COMPROBANTE_CNT.valor_credito - DETALLE_COMPROBANTE_CNT.valor_debito END END )
    ...
    ]') || TO_CLOB(q'[                       AND SA.CODIGO = DIARIO.CUENTA
                ...
                LEFT JOIN
                         (SELECT DETALLE_COMPROBANTE_CNT.COMPANIA,
                          ...
                          FROM COMPROBANTE_CNT CNT INNER JOIN TIPO_COMPROBANTE TIP
                           ...
                           INNER JOIN]') || TO_CLOB(q'[ DETALLE_COMPROBANTE_CNT
...
WHERE P.COMPANIA = s$compania$s
 ...
 AND (ABS(SA.SALDOs$mesAnterior$s) + ABS(NVL(DIARIO.SALDOALDIA,0)) + ABS(NVL(DE.VALOR_DEBITO,0)) + ABS(NVL(DE.VALOR_CREDITO,]') || TO_CLOB(q'[0)) ) <>0
...
CASE WHEN INSTR('ILB',DE.CLASE_CONTABLE)<>0 THEN 'E' ELSE 'S' END,
        ...
        END,
        P.NATURALEZA) TABLA
GROUP BY NOCOMP, NOANO, CODIGO, VD, VC,
 ...
 CENTRO_COSTO, NATURALEZA]') CONSULTA, 1 APLICACION ,TO_CLOB(q'[]') CONSULTA_OPCIONAL, NULL CREATED_BY, NULL MODIFIED_BY  FROM DUAL ) INI ON (INI.INFORME = FIN.INFORME )  WHEN MATCHED THEN  UPDATE SET FIN.CONSULTA =  INI.CONSULTA, FIN.APLICACION =  INI.APLICACION, FIN.CONSULTA_OPCIONAL =  INI.CONSULTA_OPCIONAL, FIN.MODIFIED_BY = INI.MODIFIED_BY, FIN.DATE_MODIFIED = SYSDATE  WHEN NOT MATCHED THEN  INSERT (INFORME,CONSULTA, APLICACION,CONSULTA_OPCIONAL,CREATED_BY,DATE_CREATED)  VALUES (INI.INFORME,INI.CONSULTA, INI.APLICACION,INI.CONSULTA_OPCIONAL,INI.CREATED_BY,SYSDATE);