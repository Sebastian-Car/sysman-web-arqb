MERGE INTO CONSULTAS FIN USING (SELECT '002674AUXILIARESPORAUXREFERENCIADOESPECIAL' INFORME ,TO_CLOB(q'[SELECT  COMPANIA,
        CODIGOPLAN CUENTA,
...
SELECT
       ...
       END NOMBRETERCERO,
       P.SALDOs$mesAnterior$s SALDOINI,
       ...
       END TERCERO
FROM PLAN_CO]') || TO_CLOB(q'[NTABLE P INNER JOIN   SALDO_AUX_CONTABLE SA
   ...
   LEFT JOIN ( SELECT COMPANIA,ANO,CUENTA, SUM( CASE WHEN  s$fechaInicial$s =  TO_DATE('01'||'/'||TO_CHAR(s$fechaInicial$s,'MM')||'/'||TO_CHAR(s$fechaInicial$s,'YYYY'), 'DD/MM/YYYY' )THEN 0 ELSE  CASE WHEN DETALLE_COMPROBANTE_CNT.naturaleza = 'D' THEN DETALLE_COMPROBANTE_CNT.valor_debito - DETALLE_COMPROBANTE_CNT.valor_credito ELSE DETALLE_COMPROBANTE_CNT.valor_credito - DETALLE_COMPROBANTE_CNT.valor_debito END END )
                ...
                LEFT JOIN
                 ...
                 DETALLE_COMPROBANTE_CNT.CMPTE_AFECTADO,
                            ...
                            ]') || TO_CLOB(q'[      CASE WHEN DETALLE_COMPROBANTE_CNT.FECHA>= s$fechaInicial$s THEN
  ...
  ]') || TO_CLOB(q'[                       AND SA.TERCERO        = DE.TERCEROI
...
NOMBRETERCERO, NOMBREPLAN, CODIGOREF, TERCERO, 0, CONSECUTIVO,
CENTRO_COSTO ORDER BY FECHA ASC]') CONSULTA, 1 APLICACION ,TO_CLOB(q'[]') CONSULTA_OPCIONAL, 'MPEREZ' CREATED_BY, NULL MODIFIED_BY  FROM DUAL ) INI ON (INI.INFORME = FIN.INFORME )  WHEN MATCHED THEN  UPDATE SET FIN.CONSULTA =  INI.CONSULTA, FIN.APLICACION =  INI.APLICACION, FIN.CONSULTA_OPCIONAL =  INI.CONSULTA_OPCIONAL, FIN.MODIFIED_BY = INI.MODIFIED_BY, FIN.DATE_MODIFIED = SYSDATE  WHEN NOT MATCHED THEN  INSERT (INFORME,CONSULTA, APLICACION,CONSULTA_OPCIONAL,CREATED_BY,DATE_CREATED)  VALUES (INI.INFORME,INI.CONSULTA, INI.APLICACION,INI.CONSULTA_OPCIONAL,INI.CREATED_BY,SYSDATE);