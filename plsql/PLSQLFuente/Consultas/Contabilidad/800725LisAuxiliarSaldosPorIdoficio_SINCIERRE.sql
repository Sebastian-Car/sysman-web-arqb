MERGE INTO CONSULTAS FIN USING (SELECT '800725LisAuxiliarSaldosPorIdoficio_SINCIERRE' INFORME ,TO_CLOB(q'[SELECT 
    TABLA.COD_CUENTA
    ,TABLA.NOMBRE_CUENTA
    ,TABLA.FECHA
    ,TABLA.HORA
...
FROM (
        ...
        OVER(
            ORDER BY
                ...
                ELSE TRIM(RPAD(SA.CODIGO,32)
                    ...
                    || RPAD(CASE WHEN MAN_AUX_TER IN(0) THEN ' ' ELSE SA.TERCERO        END  ,18) 
                    || RPAD(CASE WHEN MAN_AUX_TER IN(0) THEN ' ' ELSE SA.SUCURSAL       END  ,3) 
   ]') || TO_CLOB(q'[                 || RPAD(CASE WHEN MAN_AUX_GEN IN(0) THEN ' ' ELSE SA.AUXILIAR       END  ,20) 
                    || RPAD(CASE WHEN MAN_AUX_REF IN(0) THEN ' ' ELSE SA.REFERENCIA     END  ,20) 
                    || RPAD(CASE WHEN MAN_AUX_FUE IN(0) THEN ' ' ELSE SA.FUENTE_RECURSO END  ,20) 
                )END ASC,
    ...
    LEFT JOIN (
        SELECT DETALLE_COMPROBANTE_CNT.COMPANIA, 
            ...
            ]') || TO_CLOB(q'[DETALLE_COMPROBANTE_CNT.SUCURSALI,
        ...
        INNER JOIN REFERENCIA REFERENCIA_DET
            ...
            --AND DETALLE_COMPROBANTE_CNT.TERCERO ]') || TO_CLOB(q'[BETWEEN 'S$TERCEROINICIAL$S' AND 'S$TERCER1OFINAL$S' 
...
ORDER BY 
    ...
    ,TO_DATE(HORA,'HH24:MI') ASC]') CONSULTA, 1 APLICACION ,TO_CLOB(q'[]') CONSULTA_OPCIONAL, NULL CREATED_BY, NULL MODIFIED_BY  FROM DUAL ) INI ON (INI.INFORME = FIN.INFORME )  WHEN MATCHED THEN  UPDATE SET FIN.CONSULTA =  INI.CONSULTA, FIN.APLICACION =  INI.APLICACION, FIN.CONSULTA_OPCIONAL =  INI.CONSULTA_OPCIONAL, FIN.MODIFIED_BY = INI.MODIFIED_BY, FIN.DATE_MODIFIED = SYSDATE  WHEN NOT MATCHED THEN  INSERT (INFORME,CONSULTA, APLICACION,CONSULTA_OPCIONAL,CREATED_BY,DATE_CREATED)  VALUES (INI.INFORME,INI.CONSULTA, INI.APLICACION,INI.CONSULTA_OPCIONAL,INI.CREATED_BY,SYSDATE);