MERGE INTO CONSULTAS FIN USING (SELECT '800717AuxiliarSaldoCeroRefEspecial_SINCHI' INFORME ,TO_CLOB(q'[SELECT  NOCOMP,
       NOANO,
       ID,
       VD,
       VC,
...
FROM (
  SELECT
       ...
       P.ANO NOANO,
       P.CODIGO CUENTA, 
       P.NOMBRE NOMBRE_CUENTA,
       SA.CENTRO_COSTO CODIGO_CENTRO_COSTO,
       TRIM(RPAD(SA.CODIGO,32)
       ...
       COALESCE(DE.TERCERO_NOM, TERCERO_SA.NOMBRE)
       NOMBRETERCERO,
       SA.SALDOs$mesAnterior$s  SALDOINI,
       ...
       COALESCE(DE.TERCERO, SA.TERCERO)
       TERCERO
  ...
  LEFT JOIN (
    SELECT COMPANIA, ANO, CUENTA,
      SUM(
        CASE
          WHEN TO_DATE(s$fechaInicial$s, 'DD/MM/YYYY') = TO_DATE('01/' || TO_CHAR(TO_DATE(s$fechaInicial$s, 'DD/MM/YYYY'), 'MM') || '/' || TO_CHAR(TO_DATE(s$fechaInicial$s, 'DD/MM/YYYY'), 'YYYY'), 'DD/MM/YYYY')
            THEN 0
          ELSE
            CASE
              WHEN DETALLE_COMPROBANTE_CNT.naturaleza = 'D'
      ]') || TO_CLOB(q'[          THEN DETALLE_COMPROBANTE_CNT.valor_debito - DETALLE_COMPROBANTE_CNT.valor_credito
    ...
    WHERE COMPANIA = s$compania$s
      AND ANO = s$anio$s
      AND DETALLE_COMPROBANTE_CNT.FECHA >= TO_DATE('01/' || TO_CHAR(TO_DATE(s$fechaInicial$s, 'DD/MM/YYYY'), 'MM') || '/' || TO_CHAR(TO_DATE(s$fechaInicial$s, 'DD/MM/YYYY'), 'YYYY'), 'DD/MM/YYYY')
      AND DETALLE_COMPROBANTE_CNT.FECHA < TO_DATE(s$fechaInicial$s, 'DD/MM/YYYY')
      AND CUENTA BETWEEN 's$cuentaInicial$s' AND 's$cuentaFinal$s'
    GROUP BY COMPANIA, ANO, CUENTA
  ) DIARIO
  ...
  LEFT JOIN (
    SELECT
      ...
      CASE WHEN DETALLE_COMPROBANTE_CNT.FECHA >= TO_DATE(s$fechaInicial$s, 'DD/MM/YYYY')
    ...
    INNER JOIN TIPO_COMPROBANTE TIP
      ON CNT.COMPANIA = TIP.COMPANIA AND CNT.TIPO = TIP.CODIGO
    ...
    WHERE C]') || TO_CLOB(q'[NT.COMPANIA = s$compania$s
      ...
      AND TERCERO_DET.NOMBRE BETWEEN 's$terceroInicial$s' AND 's$terceroFinal$s'
