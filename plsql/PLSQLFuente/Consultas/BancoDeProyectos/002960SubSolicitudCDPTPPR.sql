MERGE INTO CONSULTAS FIN USING (SELECT '002960SubSolicitudCDPTPPR' INFORME ,TO_CLOB(q'[WITH saldos AS (
    SELECT
        compania,
        ano,
        id,
...
FROM
    ...
    INNER JOIN referencia ON v_solicitud_cdp.compania = referencia.compania
                        AND v_solicitud_cdp.VIGENCIA  = referencia.ano
                             AND v_so]') || TO_CLOB(q'[licitud_cdp.referencia = referencia.codigo
...
WHERE
     ...
     AND V_SOLICITUD_CDP.DEPENDENCIA   = s$dependencia$s]') CONSULTA, 52 APLICACION ,TO_CLOB(q'[]') CONSULTA_OPCIONAL, NULL CREATED_BY, NULL MODIFIED_BY  FROM DUAL ) INI ON (INI.INFORME = FIN.INFORME )  WHEN MATCHED THEN  UPDATE SET FIN.CONSULTA =  INI.CONSULTA, FIN.APLICACION =  INI.APLICACION, FIN.CONSULTA_OPCIONAL =  INI.CONSULTA_OPCIONAL, FIN.MODIFIED_BY = INI.MODIFIED_BY, FIN.DATE_MODIFIED = SYSDATE  WHEN NOT MATCHED THEN  INSERT (INFORME,CONSULTA, APLICACION,CONSULTA_OPCIONAL,CREATED_BY,DATE_CREATED)  VALUES (INI.INFORME,INI.CONSULTA, INI.APLICACION,INI.CONSULTA_OPCIONAL,INI.CREATED_BY,SYSDATE);