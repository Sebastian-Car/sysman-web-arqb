MERGE INTO CONSULTAS FIN USING (SELECT '8000578Fcregistroreservapptal036_Plano' INFORME ,TO_CLOB(q'[SELECT 
  ...
  CASE WHEN (CASE WHEN (CASE WHEN CLASE IN('RES','DMR','ADR')        THEN '7'        ELSE ''        END||CASE WHEN CLASE IN ('REO','MRO','ARO','DRO')                  THEN '8'                  ELSE ''                  END||CASE WHEN CLASE IN ('EGR','AEG','DEG')                            THEN '10'                            ELSE ''                            END) ='7'        THEN 'RC'        ELSE ''        END||CASE WHEN (CA]') || TO_CLOB(q'[SE WHEN CLASE IN('RES','DMR','ADR')        THEN '7'        ELSE ''        END||CASE WHEN CLASE IN ('REO','MRO','ARO','DRO')                  THEN '8'                  ELSE ''                  END||CASE WHEN CLASE IN ('EGR','AEG','DEG')                            THEN '10'                            ELSE ''                            END)='8'                  THEN 'OC'                  ELSE ''                  END||CASE WHEN (CASE WHEN CLASE IN('RES','DMR','ADR')        THEN '7'        ELSE ''        END||CASE WHEN CLASE IN ('REO','MRO','ARO','DRO')                  THEN '8'                  ELSE ''                  END||CASE WHEN CLASE IN ('EGR','AEG','DEG')                            THEN '10'                            ELSE ''                            END)='10'                            THEN 'TP'                            ELSE ''                            END)='RC'
       THEN 1
  ...
  CASE WHEN (DETALLE_COMPROBANTE_PPTAL.VALOR_DEBITO-DETALLE_COMPROBANTE_PPTAL.VALOR_CREDITO)<0
       ...
       ELSE '+'
       END SIGNO,
  CASE WHEN CLASE IN('RES','DMR',']') || TO_CLOB(q'[ADR')
       THEN '7'
       ELSE ''
...
WHERE DETALLE_COMPROBANTE_PPTAL.COMPANIA = s$compania$s
  ...
  AND (CASE WHEN (CASE WHEN CLASE IN('RES','DMR','ADR')        THEN '7'        ELSE ''        END||CASE WHEN CLASE IN ('REO','MRO','ARO','DRO')                  THEN '8'                  ELSE ''                  END||CASE WHEN CLASE IN ('EGR','AEG','DEG')                            THEN '10'                            ELSE ''                            END) ='7'
       THEN 'RC'
       ELSE ''
       END||CASE WHEN (CASE WHEN CLASE IN('RES','DMR','ADR')        THEN '7'        ELSE ''        END||CASE WHEN CLASE IN ('REO','MRO','ARO','DRO')                  THEN '8'                  ELSE ''                  END||CASE WHEN CLASE IN ('EGR','AEG','DEG')                            THEN '10'                            ELSE ''               ]') || TO_CLOB(q'[             END)='8'
                 THEN 'OC'
                 ELSE ''
                 END||CASE WHEN (CASE WHEN CLASE IN('RES','DMR','ADR')        THEN '7'        ELSE ''        END||CASE WHEN CLASE IN ('REO','MRO','ARO','DRO')                  THEN '8'                  ELSE ''                  END||CASE WHEN CLASE IN ('EGR','AEG','DEG')                            THEN '10'                            ELSE ''                            END)='10'
                           THEN 'TP'
                           ELSE ''
                           END) IS NOT NULL]') CONSULTA, 3 APLICACION ,TO_CLOB(q'[]') CONSULTA_OPCIONAL, NULL CREATED_BY, NULL MODIFIED_BY  FROM DUAL ) INI ON (INI.INFORME = FIN.INFORME )  WHEN MATCHED THEN  UPDATE SET FIN.CONSULTA =  INI.CONSULTA, FIN.APLICACION =  INI.APLICACION, FIN.CONSULTA_OPCIONAL =  INI.CONSULTA_OPCIONAL, FIN.MODIFIED_BY = INI.MODIFIED_BY, FIN.DATE_MODIFIED = SYSDATE  WHEN NOT MATCHED THEN  INSERT (INFORME,CONSULTA, APLICACION,CONSULTA_OPCIONAL,CREATED_BY,DATE_CREATED)  VALUES (INI.INFORME,INI.CONSULTA, INI.APLICACION,INI.CONSULTA_OPCIONAL,INI.CREATED_BY,SYSDATE);