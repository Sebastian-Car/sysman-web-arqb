MERGE INTO CONSULTAS FIN USING (SELECT '800724DISRESREOEGR_SIN_PAGOS_REPETIDOS' INFORME ,TO_CLOB(q'[WITH JERARQUIA AS (SELECT DCP.COMPANIA,
                          DCP.ANO,
                          DCP.TIPO_CPTE,
                          DCP.COMPROBANTE,
                          DCP.ANO_AFECT,
    ...
    SELECT  DCPP.COMPANIA, 
      ...
      INNER JOIN DETALLE_COMPROBANTE_PPTAL DCPP 
        ...
        AND JQ.ANO= DCPP.ANO 
        AND JQ.TIPO_CPTE=DCPP.TI]') || TO_CLOB(q'[PO_CPTE 
...
DIS_SUB AS (
    SELECT
        DIS.*,
        ROW_NUMBER() OVER (
            PARTITION BY DIS.COMPANIA, DIS.COMPROBANTE, DIS.VALOR
            ORDER BY DIS.CUENTA
        ) AS RN
    FROM RESUMEN DIS
)
SELECT 
    ...
    CASE WHEN DIS.RN = 1 THEN DIS.VALOR ELSE 0 END VALOR_DIS, -- <--- AJUSTE AQUÍ
    RES.ANO            ANO_RES,
    RES.COMPROBANTE    RES, 
    RES.TIPO           TIPO_RES,
    RES.FECHA          FECHA_RES, 
