MERGE INTO CONSULTAS FIN USING (SELECT '000015ListadoBAN' INFORME ,TO_CLOB(q'[SELECT DISTINCT PERSONAL.NOMBRECOMPLETO NOMBRECOMP, 
        ...
        PERSONAL.COMPANIA, 
        PCK_SYSMAN_UTL.FC_STRZERO(TO_CHAR(PERSONAL.INGRESO_DISTRITO,'MM'),2) AN
FROM PERSONAL 
  LEFT JOIN CATEGORIA 
...
WHERE PERSONAL.COMPANIA      =  s$compania$s 
  AND PCK_SYSMAN_UTL.FC_STRZERO(TO_CHAR(PERSONAL.INGRESO_DISTRITO,'MM'),2) = s$mes1$s 
  AND PERSONAL.ID_DE_EMPLEADO    NOT IN (0) 
  AND PERSONAL.ESTADO_ACTUAL     NOT IN (3,2) 
  AND CATEGORIA.ANO              =   s$anio$s 
  AND EXTRACT(YEAR FROM PERSONAL.FECHA_DE_INGRESO) NOT IN (s$anio$s) 
