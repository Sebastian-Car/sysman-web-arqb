MERGE INTO CONSULTAS FIN USING (SELECT '800168InformeF07RelacionDePagos' INFORME ,TO_CLOB(q'[SELECT TO_CHAR(DC.FECHA,'DD/MM/YYYY')                             FECHA_DE_PAGO,
      ...
      NVL(EGRESOS_TESORERIA.SEGURIDAD_SOCIAL,0)                   SEGURIDAD_SOCIAL,
...
LEFT JOIN (SELECT DISTINCT DC.COMPANIA,
          ...
          FROM  V_DETALLE_AUXILIAR_CNT DC 
            INNER]') || TO_CLOB(q'[ JOIN COMPROBANTE_PPTAL  CP
             ...
             LEFT JOIN (SELECT DC.COMPANIA,
                       ...
                       FROM DETALLE_COMPROBANTE_CNT DC 
                                ...
                                WHERE DC.COMPANIA=s$compania$s
                                     AND DC.ANO=s$ano$s
                                     AND DC.MES BETW]') || TO_CLOB(q'[EEN s$mesInicial$s AND s$mesFinal$s
...
GROUP BY DC.FECHA,                          
      ...
      ORDER BY DC.FECHA]') CONSULTA, 99 APLICACION ,TO_CLOB(q'[]') CONSULTA_OPCIONAL, NULL CREATED_BY, NULL MODIFIED_BY  FROM DUAL ) INI ON (INI.INFORME = FIN.INFORME )  WHEN MATCHED THEN  UPDATE SET FIN.CONSULTA =  INI.CONSULTA, FIN.APLICACION =  INI.APLICACION, FIN.CONSULTA_OPCIONAL =  INI.CONSULTA_OPCIONAL, FIN.MODIFIED_BY = INI.MODIFIED_BY, FIN.DATE_MODIFIED = SYSDATE  WHEN NOT MATCHED THEN  INSERT (INFORME,CONSULTA, APLICACION,CONSULTA_OPCIONAL,CREATED_BY,DATE_CREATED)  VALUES (INI.INFORME,INI.CONSULTA, INI.APLICACION,INI.CONSULTA_OPCIONAL,INI.CREATED_BY,SYSDATE);