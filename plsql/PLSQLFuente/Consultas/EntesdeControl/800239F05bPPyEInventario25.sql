MERGE INTO CONSULTAS FIN USING (SELECT '800239F05bPPyEInventario25' INFORME ,TO_CLOB(q'[SELECT     CODIGO                   "Código Contable",
           NOMBRE                   "Nombre De La Cuenta",
           SALDOANT                 "Saldo Inicial",   
           DEBITO                   "Entradas", 
           CREDITO                  "Salidas"
     ...
     FROM (   
        SELECT     CODIGO,                                                                                            
                 ...
                 (CASE WHEN 0  BETWEEN s$mesInicial$s AND s$mesFinal$s  THEN PLAN_CONTABLE.DEBITO0  ELSE 0 END
                   ...
                   + CASE WHEN 9  BETWEEN s$mesInicial$s A]') || TO_CLOB(q'[ND s$mesFinal$s  THEN PLAN_CONTABLE.DEBITO9  ELSE 0 END
           ...
           WHERE PLAN_CONTABLE.COMPANIA =s$compania$s
             AND PLAN_CONTABLE.ANO      =s$ano$s
              AND PLAN_CONTABLE.INFORME_SIA = -1
           )
WHERE     SALDOANT +  DEBITO  +  CREDITO  <>0]') CONSULTA, 99 APLICACION ,TO_CLOB(q'[]') CONSULTA_OPCIONAL, NULL CREATED_BY, NULL MODIFIED_BY  FROM DUAL ) INI ON (INI.INFORME = FIN.INFORME )  WHEN MATCHED THEN  UPDATE SET FIN.CONSULTA =  INI.CONSULTA, FIN.APLICACION =  INI.APLICACION, FIN.CONSULTA_OPCIONAL =  INI.CONSULTA_OPCIONAL, FIN.MODIFIED_BY = INI.MODIFIED_BY, FIN.DATE_MODIFIED = SYSDATE  WHEN NOT MATCHED THEN  INSERT (INFORME,CONSULTA, APLICACION,CONSULTA_OPCIONAL,CREATED_BY,DATE_CREATED)  VALUES (INI.INFORME,INI.CONSULTA, INI.APLICACION,INI.CONSULTA_OPCIONAL,INI.CREATED_BY,SYSDATE);