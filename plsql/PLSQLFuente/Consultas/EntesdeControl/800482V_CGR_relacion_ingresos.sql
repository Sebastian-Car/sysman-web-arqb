MERGE INTO CONSULTAS FIN USING (SELECT '800482V_CGR_relacion_ingresos' INFORME ,TO_CLOB(q'[SELECT
    ...
    NVL(PPC.CODIGO_CCEPT,D.CUENTA) CODIGO_PRESUPUESTAL,    
...
FROM
    ...
    INNER JOIN 
        (SELECT COMPANIA, ANO, TIPO_CPTE,FECHA, MES, COMPROBANTE, CUENTA, TERCERO, SUCURSAL,
         ...
         WHERE COMPANIA = s$compania$s
           AND ANO=s$ano$s 
         GROUP BY COMPANIA, ANO, TIPO_CPTE,FECHA, MES, COMPROBANTE, CUENTA, TERCERO, SUCURSAL,
                TERCEROI, SUCURSALI, AUXILIARI, CENTRO_COSTOI, FUENTE_RECURSOI, REFERENCIAI
    ...
    INNER JOIN PLAN_PPTAL_CONFIG           PPC ON D.COMPANIA = PPC.COMPANIA

                                        AND D.ANO = PPC.ANO
                                        AND D.CUENTA = PPC.CODIGO
                                        AND D.TERCEROI = PPC.TERCERO
                                        AND D.SUCURSALI = PPC.SUCURSAL
                                        AND D.AUXILIARI = PPC.AUXILIAR
                                        AND D.CENTRO_COSTOI = PPC.CENTRO_COSTO
                                        AND D.FUENTE_RECURSOI = PPC.FUENTE_RECURSO
                                        AND D.REFERENCIAI = PPC.REFERENCIA
...
WHERE
    ...
    AND PLAN_PRESUPUESTAL.REGALIAS IN (
        0
    )
ORDER BY
    TO_CHAR(D.FECHA, 'YYYY-MM-DD')]') CONSULTA, 99 APLICACION ,TO_CLOB(q'[]') CONSULTA_OPCIONAL, 'rmedica' CREATED_BY, NULL MODIFIED_BY  FROM DUAL ) INI ON (INI.INFORME = FIN.INFORME )  WHEN MATCHED THEN  UPDATE SET FIN.CONSULTA =  INI.CONSULTA, FIN.APLICACION =  INI.APLICACION, FIN.CONSULTA_OPCIONAL =  INI.CONSULTA_OPCIONAL, FIN.MODIFIED_BY = INI.MODIFIED_BY, FIN.DATE_MODIFIED = SYSDATE  WHEN NOT MATCHED THEN  INSERT (INFORME,CONSULTA, APLICACION,CONSULTA_OPCIONAL,CREATED_BY,DATE_CREATED)  VALUES (INI.INFORME,INI.CONSULTA, INI.APLICACION,INI.CONSULTA_OPCIONAL,INI.CREATED_BY,SYSDATE);