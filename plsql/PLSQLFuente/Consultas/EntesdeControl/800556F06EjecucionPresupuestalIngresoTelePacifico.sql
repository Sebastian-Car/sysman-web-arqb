MERGE INTO CONSULTAS FIN USING (SELECT '800556F06EjecucionPresupuestalIngresoTelePacifico' INFORME ,TO_CLOB(q'[SELECT 
       V_PLAN_PRESUPUESTAL.CODIGO 			"Código Rubro", 
       V_PLAN_PRESUPUESTAL.NOMBRE 					                        "Descripcion Ingreso",
       SUM(SALDOS.APROPIACIONANTERIOR  ) 				                    "Apropiiacion Inicial",
       SUM(SALDOS.ADICION              ) 				                    "Adiciones",
       SUM(SALDOS.REDUCCION            ) 				                    "Reducciones",
        SUM(SALDOS.CREDITOS             )                                   "Creditos",
       SUM(SALDOS.DEBITOS              )                                    "Contracreditos",
       SUM(SALDOS.DEFINITIVO           )                                    "Apropiacion Definitiva",
       SUM(SALDOS.EJECUCION            ) 				                    "Ejecucion del Periodo"
...
SELECT PLAN_PRESUPUESTAL.COMPANIA,
      ...
      SUM(SALDO_AUX_PPTAL.ADICION 
       ...
       SUM(CASE WHEN SALDO_A]') || TO_CLOB(q'[UX_PPTAL.MES BETWEEN  s$mesInicial$s AND  s$mesFinal$s 
       ...
       SUM(apropiacion_debito) DEBITOS,
       SUM(apropiacion_credito) CREDITOS     
...
AND V_PLAN_PRESUPUESTAL.ID       = SUBSTR(SALDOS.ID, 1, LENGTH(V_PLAN_PRESUPUESTAL.ID))
 GROUP BY V_PLAN_PRESUPUESTAL.CODIGO, 
