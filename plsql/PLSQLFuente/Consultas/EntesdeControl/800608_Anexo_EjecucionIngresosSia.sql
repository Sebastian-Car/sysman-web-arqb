MERGE INTO CONSULTAS FIN USING (SELECT '800608_Anexo_EjecucionIngresosSia' INFORME ,TO_CLOB(q'[SELECT 
       PLAN_PPTAL_CONFIG.CODIGO_SIA, 
       TIPOCAMPO_SIA.NOMBRECUENTA NOMBRE,
       SUM(SALDOS.APROPIACIONANTERIOR  ) 				                    Estimado_Inicial,
       SUM(SALDOS.ADICION              ) 				                    Adiciones,
       SUM(SALDOS.REDUCCION            ) 				                    Reducciones, 
       SUM(SALDOS.DEFINITIVO          )                                    Estimado_Final,    
       SUM(SALDOS.EJECUCION            ) 				                    Total_Recaudado,
       SUM(SALDOS.EJECUCION - SALDOS.DEFINITIVO)                            Saldo_por_Recaudar,
       ROUND((CASE WHEN SUM(SALDOS.DEFINITIVO)= 0
       THEN 100 
       ELSE  SUM(SALDOS.EJECUCION)/ SUM(SALDOS.DEFINITIVO)
...
SELECT PLAN_PRESUPUESTAL.COMPANIA,
      ...
      SUM(SALDO_AUX_PPTAL.ADICION 
       ...
       SUM(CASE WHEN SALDO_AUX_PPTAL.MES BETWEEN  s$mesInicia]') || TO_CLOB(q'[l$s AND  s$mesFinal$s 
...
GROUP BY PLAN_PPTAL_CONFIG.CODIGO_SIA,
         TIPOCAMPO_SIA.NOMBRECUENTA
