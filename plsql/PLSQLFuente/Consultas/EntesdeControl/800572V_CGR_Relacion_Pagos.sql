MERGE INTO CONSULTAS FIN USING (SELECT '800572V_CGR_Relacion_Pagos' INFORME ,TO_CLOB(q'[SELECT TO_CHAR(DC.ANO) Anio,
       ...
       NVL(CODIGO_CCPET.CODIGO,'N/A') Cod_Cuenta,
       REPLACE(REPLACE(NVL(FUENTE_CUIPO.NOMBRE,'N/A'), CHR(13), ' ') , CHR(10), ' ') Fuente_Financiacion,
	   REPLACE(REPLACE(NVL(TRIM(CODIGO_CCPET.NOMBRE),'N/A'), CHR(13), ' ') , CHR(10), ' ') Nombre_Cuenta,
	...
	END  Situacion_de_Fondos,
       DC.TIPO_CPTE || '-'|| NVL(TO_CHAR(DC.COMPROBANTE),0) Numero_Egreso,
       NVL(TO_CHAR(DC.FECHA,'YYYY-MM-DD'),'1900-01-01') Fecha_Egreso,
       NVL(TO_CHAR(DC.TIPO_CPTE_AFECT),0)|| '-'||NVL(TO_CHAR(DC.CMPTE_AFECTADO),0) Numero_Obligacion,
       ...
       REPLACE(round(NVL(EGRESOS_TESORERIA.RETENCION,0)/COUNT(DC.COMPANIA) OVER (Partition BY DC.compania, DC.ano, DC.TIPO_CPTE, DC.COMPROBANTE,  CODIGO_CCPET.CODIGO ),2),'.',',') Descuentos,           
...
LEFT JOIN (SELECT ]') || TO_CLOB(q'[DISTINCT CB.COMPANIA,
                 CB.ANO,
                 CB.TIPO_CPTE,
                 CB.COMPROBANTE,
          ...
          FROM       (SELECT UNICO.COMPANIA, 
             ...
             LEFT JOIN (SELECT COMPANIA,
                               ...
                               SUM(SEGURIDAD_SOCIAL) SEGURIDAD_SOCIAL]') || TO_CLOB(q'[,
