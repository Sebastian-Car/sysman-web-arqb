MERGE INTO CONSULTAS FIN USING (SELECT '800560F11EjecucionPresupuestalCuentasporPagarTelePacifico' INFORME ,TO_CLOB(q'[SELECT 
        PLAN_PPTAL_CONFIG.CODIGO_SIA || V_PLAN_PRESUPUESTAL.CODIGO       "Código Rubro Presupuestal",
        V_PLAN_PRESUPUESTAL.NOMBRE                                       "Descripción",
        SUM(SALDOS.APROPIACIONANTERIOR)                                  "Cuenta Por Pagar Constituida",
        'ND'                                                             "Acta De Cancelación",
        SUM(SALDOS.EJECUCIONMES)                                         "Pago" 
...
SELECT PLAN_PRESUPUESTAL.COMPANIA,
      ...
      SUM(SALDO_AUX_PPTAL.ADICION 
       ...
       SUM(SALDO_AUX_PPTAL.REG_CONTRACT 
               ...
               + ]') || TO_CLOB(q'[SALDO_AUX_PPTAL.MODIF_REG_NOCONT
            )  COMPROMISO,
    ...
    SUM(CASE WHEN SALDO_AUX_PPTAL.MES BETWEEN s$mesInicial$s AND  s$mesFinal$s 
       ...
       SUM(CASE WHEN PLAN_PRESUPUESTAL.NATURALEZA='D' 
                THEN ]') || TO_CLOB(q'[  SALDO_AUX_PPTAL.APLAZAM_CREDITO
                ELSE  SALDO_AUX_PPTAL.APLAZAM_DEBITO
