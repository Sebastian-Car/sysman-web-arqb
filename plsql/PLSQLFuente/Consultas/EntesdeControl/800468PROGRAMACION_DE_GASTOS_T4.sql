MERGE INTO CONSULTAS FIN USING (SELECT '800468PROGRAMACION_DE_GASTOS_T4' INFORME ,TO_CLOB(q'[SELECT CASE WHEN SUBSTR(NVL(CODIGO,0),0,3)= '2.3' 
THEN '2.99'             
...
NVL(NVL(INI.SECTOR_MAN,INI.SECTOR_PAD),0) SECTOR, 
...
MAX(CASE WHEN PADRE.CLASECLASIFICADOR ='001' THEN
PADRE.TIPOCLASIFICADOR 
ELSE 
NULL END) SECTOR_PAD,
MOV.SECTOR SECTOR_MAN,
MOV.TIPOVIGENCIA,
...
NVL(NVL(INI.SECTOR_MAN,INI.SECTOR_PAD),0) 
...
OR SUM(SALDO.Apropiacion_DEBITO + SALDO.Adicion +SALDO.Reduccion + (SALDO.TRASLADO_DEBITO- SALDO.TRASLADO_CREDITO) + (SALDO.Aplazam_Debito -SALDO.Aplazam_Credito)) <>0)
 GROUP BY CASE WHEN SUBSTR(NVL(CODIGO,0),0,3)= '2.3' 
THEN '2.99'             
ELSE NVL(CODIGO,0) END , 
s$nombre$s
VIGENCIA_GASTO,
SECCION_PRESUPUESTAL,
