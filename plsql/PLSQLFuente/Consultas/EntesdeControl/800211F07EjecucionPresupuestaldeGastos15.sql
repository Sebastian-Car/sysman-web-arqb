MERGE INTO CONSULTAS FIN USING (SELECT '800211F07EjecucionPresupuestaldeGastos15' INFORME ,TO_CLOB(q'[SELECT
        PLAN_PPTAL_CONFIG.CODIGO_SIA || V_PLAN_PRESUPUESTAL.CODIGO       "Cod Rubro Presupuestal",
        V_PLAN_PRESUPUESTAL.NOMBRE                                       "Nombre Rubro Presupuestal",
        CASE WHEN V_PLAN_PRESUPUESTAL.FUENTE_FUT IS NOT  NULL THEN
        TO_CHAR(V_PLAN_PRESUPUESTAL.FUENTE_FUT)  ELSE V_PLAN_PRESUPUESTAL.CODIGO END                  "Código Del Programa",
        SUM(SALDOS.APROPIACIONANTERIOR)                                  "Apropiación Inicial",
        SUM(SALDOS.TRASLADO_DEBITO)                                    "Credito",
        SUM(SALDOS.TRASLADO_CREDITO)                                     "Contracréditos",
        SUM(SALDOS.APLAZAMIENTO)                                         "Aplazamientos",
        SUM(SALDOS.DESAPLAZAMIENTO)                                      "Desaplazamientos",
        SUM(SALDOS.REDUCCION)                                            "Reducciones",
        SUM(SALDOS.ADICION)                                              " Adiciones",
        SUM(SALDOS.COMPROMISOMES)                                        "Compromisos RP",
        SUM(OBLIGACIONESMES)                                             " Obligaciones",
        SUM(SALDOS.EJECUCIONMES)                                         "Pagos"
...
SELECT PLAN_PRESUPUESTAL.COMPANIA,
      ...
      SUM(SALDO_AUX_PPTAL.ADICION
       ...
       SUM(CASE WHEN PLAN_PRESUPUESTAL.NATURALEZA='D'
]') || TO_CLOB(q'[                           THEN   SALDO_AUX_PPTAL.APLAZAM_DEBITO
                           ELSE  SALDO_AUX_PPTAL.APLAZAM_CREDITO
                           END
                          )
               APLAZAMIENTOACUM,
...
LEFT JOIN FUENTESFUT
    ON  V_PLAN_PRESUPUESTAL.COMPANIA = FUENTESFUT.COMPANIA
   AND V_PLAN_PRESUPUESTAL.ANO      = FUENTESFUT.ANO
   AND V_PLAN_PRESUPUESTAL.FUENTE_FUT      = FUENTESFUT.CODIGOFU_FUT
   AND V_PLAN_PRESUPUESTAL.DESTINO      = FUENTESFUT.DESTINO
 GROUP BY PLAN_PPTAL_CONFIG.CODIGO_SIA || V_PLAN_PRESUPUESTAL.CODIGO,
         ...
         V_PLAN_PRESUPUESTAL.NOMBRE,
        CASE WHEN V_PLAN_PRESUPUESTAL.FUENTE_FUT IS NOT  NULL THEN
        TO_CHAR(V_PLAN_PRESUPUESTAL.FUENTE_FUT)  ELSE V_PLAN_PRESUPUESTAL.CODIGO END,
         V_PLAN_PRESUPUESTAL.AUXILIAR,
         V_PLAN_PRESUPUESTAL.CENTRO_COSTO,
         V_PLAN_PRESUPUESTAL.FUENTE_RECURSO
 ORDER BY V_PLAN_PRESUPUESTAL.ID]') CONSULTA, 99 APLICACION ,TO_CLOB(q'[]') CONSULTA_OPCIONAL, NULL CREATED_BY, NULL MODIFIED_BY  FROM DUAL ) INI ON (INI.INFORME = FIN.INFORME )  WHEN MATCHED THEN  UPDATE SET FIN.CONSULTA =  INI.CONSULTA, FIN.APLICACION =  INI.APLICACION, FIN.CONSULTA_OPCIONAL =  INI.CONSULTA_OPCIONAL, FIN.MODIFIED_BY = INI.MODIFIED_BY, FIN.DATE_MODIFIED = SYSDATE  WHEN NOT MATCHED THEN  INSERT (INFORME,CONSULTA, APLICACION,CONSULTA_OPCIONAL,CREATED_BY,DATE_CREATED)  VALUES (INI.INFORME,INI.CONSULTA, INI.APLICACION,INI.CONSULTA_OPCIONAL,INI.CREATED_BY,SYSDATE);