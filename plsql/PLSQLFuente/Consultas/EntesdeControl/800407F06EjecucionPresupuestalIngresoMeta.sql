MERGE INTO CONSULTAS FIN USING (SELECT '800407F06EjecucionPresupuestalIngresoMeta' INFORME ,TO_CLOB(q'[SELECT 
       V_PLAN_PRESUPUESTAL.CODIGO 			"Código Rubro", 
       V_PLAN_PRESUPUESTAL.NOMBRE 					                        "Descripcion Ingreso",
       SUM(SALDOS.APROPIACIONANTERIOR  ) 				                    "Aforo Inicial",
       SUM(SALDOS.ADICION              ) 				                    "Adiciones",
       SUM(SALDOS.REDUCCION            ) 				                    "Reducciones",
       SUM(SALDOS.DEFINITIVO           )                                    "Aforo Definitivo",
       SUM(SALDOS.EJECUCION            ) 				                    "Total Recaudado",
       SUM(SALDOS.DEFINITIVO - SALDOS.EJECUCION)                            "Saldo Por Recaudar", 
       ROUND( (
               (CASE WHEN SUM(SALDOS.DEFINITIVO) = 0 
                THEN 0
                ELSE 1-SUM(SALDOS.EJECUCION)/SUM(SALDOS.DEFINITIVO) 
                END ) 
            )*100 ,2)                                                      "% Por Recaudar"
...
SELECT PLAN_PRESUPUESTAL.COMPANIA,
      ...
      SUM(SALDO_AUX_PPTAL.ADICION 
       ...
       SUM(CASE WHEN SALDO_AUX_PPTAL.MES BETWEEN  s$mesInicial$s AND  s$mesFinal$s 
                ...
                ELSE 0 END)  ADICION,
       SUM(CASE WHEN SALDO_AUX_PPTAL.MES BETWEEN  s$mesInicial$s AND  s$mesFinal$s 
                THEN SALDO_AUX_PPTAL.TRAS]') || TO_CLOB(q'[LADO_DEBITO 
