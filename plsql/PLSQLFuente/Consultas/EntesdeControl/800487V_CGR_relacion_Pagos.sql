MERGE INTO CONSULTAS FIN USING (SELECT '800487V_CGR_relacion_Pagos' INFORME ,TO_CLOB(q'[SELECT TO_CHAR(DC.ANO) Anio,
...
FROM DETALLE_COMPROBANTE_PPTAL DC 
      ...
      LEFT JOIN FUENTE_RECURSOS
                 ON DC.COMPANIA=FUENTE_RECURSOS.COMPANIA
                   ...
                   AND CASE WHEN PP.MAN_AUX_FUE =-1 THEN DC.FUENTE_RECURSO ELSE PP.FUENTE_RECURSOS END=FUENTE_RECURSOS.]') || TO_CLOB(q'[CODIGO
          ...
          FROM       (SELECT UNICO.COMPANIA, 
             ...
             LEFT JOIN (SELECT COMPANIA,
                               ...
                               COMPROBANTE_AFECT COMPROBANTE,
                                    SUM(RETENCION]') || TO_CLOB(q'[) RETENCION,
...
]') || TO_CLOB(q'[                                             AND TC.CLASE_CONTABLE IN ('P')                         
                        ) 
                        GROUP BY COMPANIA,
                               ANO_AFECT ,
                               TIPO_CPTE_AFECT ,
