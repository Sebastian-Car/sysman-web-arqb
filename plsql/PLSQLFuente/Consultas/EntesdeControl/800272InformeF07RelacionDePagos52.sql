MERGE INTO CONSULTAS FIN USING (SELECT '800272InformeF07RelacionDePagos52' INFORME ,TO_CLOB(q'[SELECT TO_CHAR(DC.FECHA,'DD/MM/YYYY')                             FECHA_DE_PAGO,
       PPC.CODIGO_SIA||DC.CUENTA                                  CODIGO_PRESUPUESTAL,
       CN.TIPOPAGO_SIA                                            TIPO_DE_PAGO,
       FR.CODIGO_SIA                                              FUENTE_DE_FINANCIACION,
      ...
      case when MAX(SUM(DC.VALOR_DEBITO-DC.VALOR_CREDITO)) OVER(PARTITION BY DC.TIPO_CPTE||'-'||DC.COMPROBANTE ) =  SUM(DC.VALOR_DEBITO-DC.VALOR_CREDITO) then   NVL(DESCUENTOS1TOTAL.SEGURIDAD_SOCIAL,0) else 0 end DESCUENTO_SEG_SOCIAL,
      case when MAX(SUM(DC.VALOR_DEBITO-DC.VALOR_CREDITO)) OVER(PARTITION BY DC.TIPO_CPTE||'-'||DC.COMPROBANTE ) =  SUM(DC.VALOR_DEBITO-DC.VALOR_CREDITO) then  NVL(DESCUENTOS1TOTAL.RETENCION,0) else 0 end DESCUENTO_RETENCION,
      case when MAX(SUM(DC.VALOR_DEBITO-DC.VALOR_CREDITO)) OVER(PARTITION BY DC.TIPO_CPTE||'-'||DC.COMPROBANTE ) =  SUM(DC.VALOR_DEBITO-DC.VALOR_CREDITO) then  NVL(DESCUENTOS1TOTAL.OTROS_DESCUENTOS,0) else 0 end OTROS_DESCUENTOS,
      SUM(DC.VALOR_DEBITO-DC.VALOR_CREDITO)- 
       ( case when MAX(SUM(DC.VALOR_DEBITO-DC.VALOR_CREDITO)) OVER(PARTITION BY DC.TIPO_CPTE||'-'||DC.COMPROBANTE ) =  SUM(DC.VALOR_DEBITO-DC.VALOR_CREDITO) then   NVL(DESCUENTOS1TOTAL.SEGURIDAD_SOCIAL,0) else 0 end)
       - (case when MAX(SUM(DC.VALOR_DEBITO-DC.VALOR_CREDITO)) OVER(PARTITION BY DC.TIPO_CPTE||'-'||DC.COMPROBANTE ) =  SUM(DC.VALOR_DEBITO-DC.VALOR_CREDITO) then  NVL(DESCUENTOS1TOTAL.RETENCION,0) else 0 end)
       - (case when MAX(SUM(DC.VALOR_DEBITO-DC.VALOR_CREDITO)) OVER(PARTITION BY DC.TIPO_CPTE||'-'||DC.COMPROBANTE ) =  SUM(DC.VALOR_DEBITO-DC.VALOR_CREDITO) then  NVL(DESCUENTOS1TOTAL.OTROS_DESCUENTOS,0) else 0 end)
       NETO_PAGO,
...
FROM DETALLE_COMPROBANTE_PPTAL DC 
      LEFT JOIN  COMPROBANTE_CNT CN
       ...
       INNER JOIN PLAN_PRESUPUESTAL PP
          ON DC.COMPANIA=PP.COMPANIA
              AND DC.ANO=PP.ANO
 ]') || TO_CLOB(q'[             AND DC.CUENTA=PP.CODIGO
    ...
    AND DC.REFERENCIA=PPC.REFERENCIA      
...
FROM (SELECT DET.COMPANIA, 
      ...
      WHERE DET.COMPANIA=s$compania$s
        ...
        AND TC.CLASE_CONTABLE IN ('E','G','A','P')
    ...
    LEFT JOIN (SELECT DC.COMPANIA,
                       ...
                       FROM DETALLE_COMPROBANTE_CNT DC 
                                  INNER JOIN PLAN_CONTABLE P
                                       ...
                                       AND DC.CUENTA=P.CODIGO
                                       AND DC.ANO=P.ANO
         ]') || TO_CLOB(q'[                         INNER JOIN TIPO_COMPROBANTE TC
                                       ON DC.COMPANIA=TC.COMPANIA
                                       AND DC.TIPO_CPTE=TC.CODIGO
                                WHERE DC.COMPANIA=s$compania$s
                                     AND DC.ANO=s$ano$s
                               ...
                               GROUP BY DC.COMPANIA,
                                        DC.ANO,
                                        DC.TIPO_CPTE,
                                        DC.COMPROBANTE) DESCUENTOS1TOTAL
                    ON DC.COMPANIA=DESCUENTOS1TOTAL.COMPANIA
                    AND DC.ANO=DESCUENTOS1TOTAL.ANO
                    AND DC.TIPO_CPTE_AFECT=DESCUENTOS1TOTAL.TIPO_CPTE
                    AND DC.CMPTE_AFECTADO=DESCUENTOS1TOTAL.COMPROBANTE
 WHERE DC.COMPANIA=s$compania$s
      AND DC.ANO=s$ano$s
...
GROUP BY DC.FECHA,                       
      ...
      DESCUENTOS1TOTAL.RETENCION,
      DESCUENTOS1TOTAL.OTROS_DESCUENTOS,
      DESCUENTOS1TOTAL.SEGURIDAD_SOCIAL]') CONSULTA, 99 APLICACION ,TO_CLOB(q'[]') CONSULTA_OPCIONAL, NULL CREATED_BY, 'mrosero' MODIFIED_BY  FROM DUAL ) INI ON (INI.INFORME = FIN.INFORME )  WHEN MATCHED THEN  UPDATE SET FIN.CONSULTA =  INI.CONSULTA, FIN.APLICACION =  INI.APLICACION, FIN.CONSULTA_OPCIONAL =  INI.CONSULTA_OPCIONAL, FIN.MODIFIED_BY = INI.MODIFIED_BY, FIN.DATE_MODIFIED = SYSDATE  WHEN NOT MATCHED THEN  INSERT (INFORME,CONSULTA, APLICACION,CONSULTA_OPCIONAL,CREATED_BY,DATE_CREATED)  VALUES (INI.INFORME,INI.CONSULTA, INI.APLICACION,INI.CONSULTA_OPCIONAL,INI.CREATED_BY,SYSDATE);