MERGE INTO CONSULTAS FIN USING (SELECT '800572V_CGR_Relacion_PagosAppUI_RUBRO' INFORME ,TO_CLOB(q'[SELECT detallePago,
Vigencia,
macroCampo,
codigoPresupuestal,
fechaPago,
...
FROM(
    SELECT  NVL(TO_CHAR(DC.COMPROBANTE),0) numeroPago,
    ...
    CASE WHEN  pp.TIPOVIGENCIA ='RA'
         ...
         '2' macroCampo,
           NVL(pp.CODIGO,'N/A') codigoPresupuestal,
           NVL(TO_CHAR(DC.FECHA,'YYYY-MM-DD'),'1900-01-01') fechaPago,
           NVL(SUM(DC.VALOR_DEBITO-DC.VALOR_CREDITO),0)  valorPago,
           round(NVL(EGRESOS_TESORERIA.RETENCION,0)/COUNT(DC.COMPANIA) OVER (Partition BY DC.compania, DC.ano, DC.TIPO_CPTE, DC.COMPROBANTE,  PP.CODIGO ),2) Descuentos,     
           NVL(DC.TERCERO,0)                                           nitTercero,
          REPLACE(REPLACE(NVL(TRIM(pp.NOMBRE),'N/A'), CHR(13), ' ') , CHR(10), ' ') nombreRubro,
    ...
    LEFT JOIN (SELECT DISTINCT CB.COMPANIA,
              ...
              FROM       (SELECT UNICO.COMPANIA, 
                                   ...
                                   UNICO.COMPROBANTE, 
                                   UNICO.CUENTA,
                                   CUENTABANCOS.CODBANCO_S]') || TO_CLOB(q'[IA,
                                   CUENTABANCOS.BANCO,
                 ...
                 LEFT JOIN (SELECT COMPANIA,
                            ...
                            FROM (
                                    SELECT DC.COMPANIA,
                                           ...
                                           EGR.cOmpRoBaNTE COMPROBAN]') || TO_CLOB(q'[TE_AFECT,
                        ...
                        AND C]') || TO_CLOB(q'[B.COMPROBANTE=DESCUENTOS.COMPROBANTE
                   LEFT JOIN BANCO
