SELECT TO_CHAR(DC.FECHA,'DD/MM/YYYY')                             "Fecha De Pago",
       PPC.CODIGO_SIA||DC.CUENTA                                  "Código Presupuestal",
       'ND'                                                       "Clase De Pago",
       CN.TIPOPAGO_SIA                                            "Tipo De Pago",
       DC.TIPO_CPTE||'-'||DC.COMPROBANTE                          "No. De Comprobante",
       T.NOMBRE                                                   "Beneficiario",
       DC.TERCERO                                                 "Cédula O Nit",
       DC.DESCRIPCION                                             "Detalle De Pago",
       SUM(DC.VALOR_DEBITO-DC.VALOR_CREDITO)                      "Valor Comprobante De Pago", 
      NVL(EGRESOS_TESORERIA.SEGURIDAD_SOCIAL,0)                   "Descuentos Seg. Social",
      NVL(EGRESOS_TESORERIA.RETENCION,0)                          "Descuentos Retenciones",
      NVL(EGRESOS_TESORERIA.OTROS,0)                              "Otros Descuentos",
      SUM(DC.VALOR_DEBITO-DC.VALOR_CREDITO)                       "Neto Pagado",
      EGRESOS_TESORERIA.CODBANCO_SIA                              "Banco",  
      EGRESOS_TESORERIA.CUENTANUMERO                              "No. De Cuenta" ,
     NVL( EGRESOS_TESORERIA.NRO_DOCUMENTO,'ND')                   "No. De Cheque O Nd"	
 ...
 WHERE ESTADO IN('A')) CB
  ...
  AND DC.COMPROBANTE=CB.COMPROBANTE
             ...
             LEFT JOIN (SELECT DC.COMPANIA,
                  ...
                  WHERE DC.COMPANIA=s$compania$s
                        AND DC.ANO=s$ano$s
                        AND DC.MESBETWEEN  s$mesInicial$s AND s$mesFinal$s
) EGRESOS_TESORERIA
