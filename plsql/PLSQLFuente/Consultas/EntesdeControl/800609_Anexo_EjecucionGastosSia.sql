MERGE INTO CONSULTAS FIN USING (SELECT '800609_Anexo_EjecucionGastosSia' INFORME ,TO_CLOB(q'[SELECT 
        PLAN_PPTAL_CONFIG.CODIGO_SIA        CONCEPTO, 
        TIPOCAMPO_SIA.NOMBRECUENTA          NOMBRE,
        SUM(SALDOS.APROPIACIONANTERIOR)     APROPIACION_INICIAL,
        SUM(SALDOS.TRASLADO_DEBITO) 		CREDITO,                   
        SUM(SALDOS.TRASLADO_CREDITO)        CONTRACREDITOS,
        SUM(DEFINITIVO)                     APROPIACION_DEFINITIVA,  
        SUM(SALDOS.COMPROMISOMES)           COMPROMISOS,
        SUM(OBLIGACIONESMES)                OBLIGACIONES,
        SUM(SALDOS.EJECUCIONMES)            PAGOS, 
        SUM(DEFINITIVO - COMPROMISOMES)     SALDO_POR_COMPROMETER,
         ROUND((CASE WHEN SUM(SALDOS.DEFINITIVO)= 0
       THEN 100 
       ELSE  SUM(SALDOS.COMPROMISOMES)/ SUM(SALDOS.DEFINITIVO)
       END) *100 ,2)                 PORC_EJECUCION  
...
SELECT PLAN_PRESUPUESTAL.COMPANIA,
      ...
      SUM(SALDO_AUX_PPTAL.ADICION 
       ...
       SUM(CASE WHEN SALDO_AUX_PPTAL.MES BETWEEN s$mesInicial$s AND  s$mesFinal$s 
                THEN SALDO_AUX_PPTAL.TRA]') || TO_CLOB(q'[SLADO_CREDITO 
       ...
       SUM(CASE WHEN PLAN_PRESUPUESTAL.NATURALEZA='C' 
               ...
               APLAZAMIENTOACUM,  
    SUM(CASE WHEN SALDO_AUX_PPTAL.MES BETWEEN s$mesInicial$s AND  s$mesFinal$s 
                THEN (CASE WHEN PLAN_PRESUPUESTAL.NATURALEZA='C]') || TO_CLOB(q'[' 
                           THEN   SALDO_AUX_PPTAL.APLAZAM_CREDITO 
 ...
 WHERE PLAN_PRESUPUESTAL.COMPANIA     = s$compania$s
  ...
  AND PLAN_PRESUPUESTAL.TIPOVIGENCIA NOT IN ('RC','RA','VT')
 ...
 INNER JOIN PLAN_PPTAL_CONFIG 
    ...
    INNER JOIN  TIPOCAMPO_SIA
ON PLAN_PPTAL_CONFIG.COMPANIA=TIPOCAMPO_SIA.COMPANIA
AND PLAN_PPTAL_CONFIG.ANO=TIPOCAMPO_SIA.ANO
AND PLAN_PPTAL_CONFIG.CODIGO_SIA=TIPOCAMPO_SIA.CODIGO
 GROUP BY PLAN_PPTAL_CONFIG.CODIGO_SIA,TIPOCAMPO_SIA.NOMBRECUENTA   
 ORDER BY PLAN_PPTAL_CONFIG.CODIGO_SIA]') CONSULTA, 99 APLICACION ,TO_CLOB(q'[]') CONSULTA_OPCIONAL, NULL CREATED_BY, NULL MODIFIED_BY  FROM DUAL ) INI ON (INI.INFORME = FIN.INFORME )  WHEN MATCHED THEN  UPDATE SET FIN.CONSULTA =  INI.CONSULTA, FIN.APLICACION =  INI.APLICACION, FIN.CONSULTA_OPCIONAL =  INI.CONSULTA_OPCIONAL, FIN.MODIFIED_BY = INI.MODIFIED_BY, FIN.DATE_MODIFIED = SYSDATE  WHEN NOT MATCHED THEN  INSERT (INFORME,CONSULTA, APLICACION,CONSULTA_OPCIONAL,CREATED_BY,DATE_CREATED)  VALUES (INI.INFORME,INI.CONSULTA, INI.APLICACION,INI.CONSULTA_OPCIONAL,INI.CREATED_BY,SYSDATE);