MERGE INTO CONSULTAS FIN USING (SELECT '800614EJEC_PRES_GAST_SALUD' INFORME ,TO_CLOB(q'[SELECT 
        PLAN_PPTAL_CONFIG.CODIGO_SUPERSALUD codGasto,
        SALDOS.FUENTE codFuente,
        SUM(SALDOS.DEFINITIVO)            pptofinal,
        SUM(SALDOS.COMPROMISO)            compromisos,
        SUM(SALDOS.OBLIGACIONES)          PAGOSTRIMESTRE,
...
SELECT PLAN_PRESUPUESTAL.COMPANIA,
      ...
      SUM(SALDO_AUX_PPTAL.ADICION 
       ...
       SUM(CASE WHEN SALDO_AUX_PPTAL.MES BETWEEN s$mesInicial$s AND s$mesFinal$s THEN
        ]') || TO_CLOB(q'[       SALDO_AUX_PPTAL.REG_CONTRACT 
               + SALDO_AUX_PPTAL.REG_NO_CONTRACT
               + SALDO_AUX_PPTAL.MODIF_REG_CONT
               + SALDO_AUX_PPTAL.MODIF_REG_NOCONT
            ELSE 0 END  
 ...
 GROUP BY PLAN_PPTAL_CONFIG.CODIGO_SUPERSALUD,
        SALDOS.FUENTE
