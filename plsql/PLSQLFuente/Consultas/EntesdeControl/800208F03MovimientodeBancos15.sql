MERGE INTO CONSULTAS FIN USING (SELECT '800208F03MovimientodeBancos15' INFORME ,TO_CLOB(q'[SELECT P.CODBANCO_SIA||' '||BN.NOMBREBANCO         " Banco",
       CUENTANUMERO                               "Numero Cuenta",
       P.NOMBRE                                     "Denominación",  
       F.NOMBRE                                     "Fuente De Financiación",
       P.SALDOs$mesInicial-1$s                      " Saldo Inicial A 1 De Enero",
       NVL(MOVIMIENTOS.INGRESOS,0)           "|D| Ingresos",
      ...
      NVL( MOVIMIENTOS.NOTAS_CREDITO,0)             "Notas Credito",
       P.SALDOs$mesFinal$s                         "Saldo Libros",
       P.AJUSTEs$mesFinal$s                         "Saldo Extractos Bancarios"
FROM  V_PLAN_CONTABLE P 
  ...
  LEFT JOIN (SELECT D.COMPANIA,
                         ...
                         SUM(CASE WHEN T.CLASE_CONTABLE In ('G','C','B')
                                  THEN VALOR_CREDITO
...
WHERE     P.COMPANIA=s$compania$s
     ...
     AND  ]') || TO_CLOB(q'[(P.SALDOs$mesInicial-1$s  + P.AJUSTEs$mesInicial-1$s     +     NVL(MOVIMIENTOS.INGRESOS,0) 
