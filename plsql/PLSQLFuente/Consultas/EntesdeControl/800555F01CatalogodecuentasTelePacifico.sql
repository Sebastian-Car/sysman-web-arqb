MERGE INTO CONSULTAS FIN USING (SELECT '800555F01CatalogodecuentasTelePacifico' INFORME ,TO_CLOB(q'[SELECT     CODIGO                   "Código Contable",
    ...
    FROM (   
        SELECT     CODIGO,                                                                                            
                 ...
                 (CASE WHEN 0  BETWEEN s$mesInicial$s AND s$mesFinal$s  THEN PLAN_CONTABLE.DEBITO0  ELSE 0 END
                   ...
                   + CASE WHEN 7  BETWEEN s$mesInicial$s AND]') || TO_CLOB(q'[ s$mesFinal$s  THEN PLAN_CONTABLE.DEBITO7  ELSE 0 END
           ...
           ]') || TO_CLOB(q'[                + CASE WHEN 4  = s$mesFinal$s  THEN PLAN_CONTABLE.SALDO4  ELSE 0 END
