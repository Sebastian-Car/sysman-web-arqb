MERGE INTO CONSULTAS FIN USING (SELECT '800572V_CGR_Relacion_PagosAppUI' INFORME ,TO_CLOB(q'[SELECT
...
FROM(
    ...
    LEFT JOIN (SELECT DISTINCT CB.COMPANIA,
              ...
              FROM       (SELECT UNICO.COMPANIA, 
                                   ...
                                   UNICO.COMPROBANTE, 
                                   UNICO.CUENTA,
                                   CUENTABANCOS.CODBAN]') || TO_CLOB(q'[CO_SIA,
                                   CUENTABANCOS.BANCO,
                 ...
                 LEFT JOIN (SELECT COMPANIA, 
                            CASE WHEN (ANO_AFECT IS NOT NULL OR ANO_AFECT <> 0) THEN ANO_AFECT ELSE ANO END ANO, 
                            CASE WHEN (TIPO_CPTE_AFECT IS NOT NULL OR TIPO_CPTE_AFECT <> 0) THEN TIPO_CPTE_AFECT ELSE TIPO_CPTE END TIPO_CPTE, 
                            CASE WHEN (COMPROBANTE_AFECT IS NOT NULL OR COMPROBANTE_AFECT <> 0) THEN COMPROBANTE_AFECT ELSE COMPROBANTE END COMPROBANTE, 
                            ...
                            FROM ( 
                                    ...
                                    DC.COMPROBANTE, 
                                    DC.CONSECUTIVO, 
            ]') || TO_CLOB(q'[                        EGR.ANO ANO_AFECT, 
                                    EGR.TIPO_CPTE TIPO_CPTE_AFECT, 
                                    EGR.COMPROBANTE COMPROBANTE_AFECT, 
                                    CASE WHEN P.TIPODESCUENTO_SIA='RETENCION' 
                                        THEN DC.VALOR_CREDITO ELSE 0 END/COUNT(DC.COMPANIA) OVER (PARTITION BY DC.COMPANIA, DC.ANO, DC.TIPO_CPTE, DC.COMPROBANTE, DC.CONSECUTIVO ) RETENCION, 
                                    CASE WHEN P.TIPODESCUENTO_SIA='SEGURIDAD_SOCIAL' 
                                        THEN DC.VALOR_CREDITO ELSE 0 END/COUNT(DC.COMPANIA) OVER (PARTITION BY DC.COMPANIA, DC.ANO, DC.TIPO_CPTE, DC.COMPROBANTE, DC.CONSECUTIVO ) SEGURIDAD_SOCIAL, 
                                    CASE WHEN P.TIPODESCUENTO_SIA='OTRO' 
                                        THEN DC.VALOR_CREDITO ELSE 0 END/COUNT(DC.COMPANIA) OVER (PARTITION BY DC.COMPANIA, DC.ANO, DC.TIPO_CPTE, DC.COMPROBANTE, DC.CONSECUTIVO ) OTROS_DESCUENTOS 
                                    FROM DETALLE_COMPROBANTE_CNT DC 
                                    INNER JOIN TIPO_COMPROBANTE TC 
                                    ON DC.COMPANIA=TC.COMPANIA 
                                    AND DC.TIPO_CPTE=TC.CODIGO 
                                    LEFT JOIN  COMPROBANTE_CNTAFECTADOS EGR 
                            ...
                            GROUP BY COMPANIA, 
                            CASE WHEN (ANO_AFECT IS NOT NULL OR ANO_AFECT <> 0) THEN ANO_AFECT ELSE ANO END, 
                            CASE WHEN (TIPO_CPTE_AFECT IS NOT NULL OR TIPO_CPTE_AFECT <> 0) THEN TIPO_CPTE_AFECT ELSE TIPO_CPTE END, 
                            CASE WHEN (COMPROBANTE_AFECT IS NOT NULL OR COMPROBANTE_AFECT <> 0) THEN COMPROBANTE_AFECT ELSE COMPROBANTE END
                    ) DESCUENTOS       
