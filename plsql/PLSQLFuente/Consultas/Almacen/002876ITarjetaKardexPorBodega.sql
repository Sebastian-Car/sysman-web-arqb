MERGE INTO CONSULTAS FIN USING (SELECT '002876ITarjetaKardexPorBodega' INFORME ,TO_CLOB(q'[WITH SALDOSANTERIORES AS (SELECT D_MOVIMIENTO.COMPANIA,
                        ...
                        SELECT  D_MOVIMIENTO.COMPANIA,
                          ...
                          AND D_MOVIMIENTO.COMPANIA = s$compania$s
                                ...
                                TO_CHAR(D_MOVIMIENTO.FECHA +
                                        ...
                                        NUMTODSINTERVAL(TO_CHAR(D_MOVIMIENTO.HORA,'ss'  ),'SECOND')
                                ,'DD/MM/YYYY hh24:mi:ss')
                                ,'DD/MM/YYYY hh24:mi:ss')          < s$fechaInicial$s
                    ...
                    AND TO_DATE(
                            TO_CHAR(D_MOVIMIENTO.FECHA +
                                    ...
                                    NUMTODSINTERVAL(TO_CHAR(D_MOVIMIENTO.HORA,'ss'  ),'SECOND')
                            ,'DD/MM/YYYY hh24:mi:ss')
                            ,'DD/MM/YYYY hh24:m]') || TO_CLOB(q'[i:ss')                 =ULTIMO.FECHA_HORA
          GROUP BY  D_MOVIMIENTO.COMPANIA,
            D_MOVIMIENTO.ELEMENTO,
            D_MOVIMIENTO.TIPOMOVIMIENTO,
            D_MOVIMIENTO.MOVIMIENTO,
   ...
   SELECT
    LPAD(V_TARJETAKARDEX.CODIGOELEMENTO,
          5) AS AGRUPACION,
           PCK_GENERALES.FC_NOMBREINVENTARIO(LPAD(V_TARJETAKARDEX.CODIGOELEMENTO,
          5) , s$compania$s , NULL) AS NOMBREINVENTARIO,
          ...
          V_TARJETAKARDEX.NOMBREDEPENDENCIA,
              ...
              CASE
                WHEN V_TARJETAKARDEX.SALDOKARDEX = 0 THEN 0
                ELSE V_TARJETAKARDEX.VALORSALDOS
              END AS VALORSALDOS,
...
WHERE V_TARJETAKARDEX.COMPANIA = s$compania$s
  ...
  AND (V_TARJETAKARDEX.BODEGA_ORIGEN = s$bodega$s OR V_TARJETAKARDEX.BODEGA_DESTINO = s$bodega$s)
group by
  LPAD(V_TARJETAKARDEX.CODIGOELEMENTO,
          5) ,
           PCK_GENERALES.FC_NOMBREINVENTARIO(LPAD(V_TARJETAKARDEX.CODIGOELEMENTO,
          5) , s$compania$s , NULL) ,
              V_TARJET]') || TO_CLOB(q'[AKARDEX.CODIGOELEMENTO,
