create or replace PACKAGE BODY PCK_CAMBIOPLACA AS
  ...
  PROCEDURE PR_INSERTAR_TABLA
       /*  
        ...
        DATE MIGRADOR     : 02/01/2020
        TIME              : 08:00 AM
        SOURCE MODULE     : 
        MODIFIER          :
        DATE MODIFIED     :
        TIME              :
        DESCRIPTION       : Permite crear un nuevo registro con la serie nueva en base a la serie anterior
                            Si la placa ya existe como parte de la llave lo consulta    
        PARAMETERS        :
        MODIFICATIONS     :

        @NAME: insertarTablaPlaca
        @METHOD:Post
*/   
    (
  ...
  BEGIN
    <<TABLASINSERT>>
    FOR RS IN ( SELECT CAMBIODEPLACA_CONFIG.* 
                FROM CAMBIODEPLACA_CONFIG INNER JOIN USER_TABLES 
                  ON CAMBIODEPLACA_CONFIG.TABLA = USER_TABLES.TABLE_NAME
               WHERE CAMBIODEPLACA_CONFIG.OPERACION =1
               ORDER BY CAMBIODEPLACA_CONFIG.ORDEN
    )LOOP      
        MI_TIENEREG :=0;
        MI_CONDICIONNUE := RS.CAMPO_COMPANIA || ' = ''' || UN_COMPANIA_NUE || '''' ||
                ' AND ' || RS.CAMPO_SERIE    || ' = ''' || UN_SERIE_NUE    || '''' ||
                ' AND ' || RS.CAMPO_ELEMENTO || ' = ''' || UN_ELEMENTO_NUE || '''';
        MI_CONDICIONANT := RS.CAMPO_COMPANIA || ' = ''' || UN_COMPANIA_ANT || '''' ||
                ' AND ' || RS.CAMPO_SERIE    || ' = ''' || UN_SERIE_ANT    || '''' ||
                ' AND ' || RS.CAMPO_ELEMENTO || ' = ''' || UN_ELEMENTO_ANT || '''';
        /**
        * VALIDA QUE EXISTAN REGISTROS EN LA TABLA CON LA PLACA ANTERIOR
        **/            
        ...
        IF MI_TIENEREG <>0 THEN
            MI_TIENEREG:= 1;
            IF INSTR('''' || REPLACE(TRIM(RS.LLAVES),',',''',''') || '''','''' || RS.CAMPO_ELEMENTO || '''',1) > 0  THEN
            ...
            IF MI_TIENEREG=1 THEN
                MI_EXCLUIDOS := RS.CAMPO_COMPANIA || ',' || RS.CAMPO_SERIE || ',' || RS.CAMPO_ELEMENTO ||',CREATED_BY,DATE_CREATED,MODIFIED_BY,DATE_MODIFIED';
...
END PR_INSERTAR_TABLA;
  ...
  PROCEDURE PR_ACTUALIZAR_TABLA
       /*  
        ...
        DATE MODIFIED     :
        TIME              :
        DESCRIPTION       : Permite actualizar el registro de una tabla que tiene una placa
                            Si la placa ya existe como parte de la llave se debe tratar
        PARAMETERS        :
        MODIFICATIONS     :

        @NAME:actualizarTablaPlaca
        @METHOD:Post
*/   
    (
    UN_COMPANIA_ANT  IN PCK_SUBTIPOS.TI_COMPANIA,
  ...
  BEGIN
    ...
    FOR RS IN ( SELECT CAMBIODEPLACA_CONFIG.* 
                FROM CAMBIODEPLACA_CONFIG INNER JOIN USER_TABLES 
                  ON CAMBIODEPLACA_CONFIG.TABLA = USER_TABLES.TABLE_NAME
               WHERE CAMBIODEPLACA_CONFIG.OPERACION = 2
               ORDER BY CAMBIODEPLACA_CONFIG.ORDEN
    )LOOP  
        BEGIN 
            MI_TIENEREG :=0;
            ...
            IF MI_TIENEREG <> 0 THEN
                ...
                MI_STRSQL:='SELECT ' || MI_CAMPOS3  ||
                          ...
                          ' WHERE COMPANIA                   =' || CHR(39) || UN_COMPANIA_ANT || CHR(39) ||
                            ' AND ' || RS.CAMPO_ELEMENTO || '=' || CHR(39) || UN_ELEMENTO_ANT || CHR(39) ||
                            ' AND ' || RS.CAMPO_SERIE    || '=' || CHR(39) || UN_SERIE_ANT    || CHR(39);
                ...
                LOOP
                    ...
                    WHILE MI_K<MI_N LOOP
                        ...
                        */
                        IF L_DESCRIPTION_TABLE(MI_K).COL_TYPE IN (1,9) THEN
                            MI_CAMPOS2:=REPLACE(MI_CAMPOS2,'CAMPO'||MI_K||' ',CHR(39)||TRIM(L_COLUMN_VALUE)||CHR(39));
                    ...
                    MI_STRSQL:='UPDATE ' || RS.TABLA ||
                                ' SET '||RS.CAMPO_ELEMENTO ||'='||CHR(39)||UN_ELEMENTO_NUE   ||CHR(39)||
                                    ','||RS.CAMPO_SERIE||'='||CHR(39)||UN_SERIE_NUE  ||CHR(39)||
                                ' WHERE '||MI_CAMPOS2;
                    */
                    MI_TABLA  := RS.TABLA;
                    MI_CAMPOS := RS.CAMPO_ELEMENTO || '=' || CHR(39) || UN_ELEMENTO_NUE ||CHR(39) ||
                           ','|| RS.CAMPO_SERIE    || '=' || CHR(39) || UN_SERIE_NUE    ||CHR(39);
                    ...
                    EXCEPTION WHEN PCK_EXCEPCIONES.EXC_ACTUALIZAR THEN
                        ...
                        MI_MSGERROR(3).VALOR := UN_ELEMENTO_ANT;
                        MI_MSGERROR(4).CLAVE := 'NIT_NUEVO';
                        MI_MSGERROR(4).VALOR := UN_ELEMENTO_NUE;
                        PCK_ERR_MSG.RAISE_WITH_MSG(UN_EXC_COD     => SQLCODE,
...
END PR_ACTUALIZAR_TABLA;
  ...
  PROCEDURE FC_ELIMINAR_TABLA
        ...
        DATE MODIFIED     :
        TIME              :
        DESCRIPTION       : Permite eliminar el registro de una tabla que tiene una placa de almacen                          
        PARAMETERS        :
        MODIFICATIONS     :

        @NAME:eliminarTablaPlaca
        @METHOD:Post
*/   
    (
    UN_COMPANIA_ANT  IN PCK_SUBTIPOS.TI_COMPANIA,
  ...
  BEGIN
    ...
    FOR RS IN ( SELECT CAMBIODEPLACA_CONFIG.* 
                FROM CAMBIODEPLACA_CONFIG INNER JOIN USER_TABLES 
                  ON CAMBIODEPLACA_CONFIG.TABLA = USER_TABLES.TABLE_NAME
               WHERE CAMBIODEPLACA_CONFIG.OPERACION =1
               ORDER BY CAMBIODEPLACA_CONFIG.ORDEN DESC
    )LOOP      
        MI_CONDICIONANT := RS.CAMPO_COMPANIA || ' = ''' || UN_COMPANIA_ANT || '''' ||
                ' AND ' || RS.CAMPO_SERIE    || ' = ''' || UN_SERIE_ANT    || '''' ||
                ' AND ' || RS.CAMPO_ELEMENTO || ' = ''' || UN_ELEMENTO_ANT || '''';
