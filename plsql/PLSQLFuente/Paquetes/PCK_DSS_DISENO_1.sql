...
*/
  ...
  BEGIN  
    ...
    MI_SALIDA :='package com.sysman.' || UN_PAQUETEJAVA || '.ejb.impl;' || CHR(13)||CHR(10) ||
                ...
                'import com.sysman.interceptor.BusinessInterceptor;' || CHR(13)||CHR(10) ||
                ...
                'import javax.ejb.EJB;'  || CHR(13)||CHR(10) ||     
                'import javax.ejb.LocalBean;' || CHR(13)||CHR(10) ||
                'import javax.ejb.Stateless;' || CHR(13)||CHR(10) || 
                'import javax.interceptor.Interceptors;'  || CHR(13)||CHR(10) ||     CHR(13)||CHR(10) ||
                '/**' || CHR(13)||CHR(10) ||
                ' * Session Bean implementation class ' || UN_NOMBRE_CLASEJAVA || '' || CHR(13)||CHR(10) ||
                ' */' || CHR(13)||CHR(10) ||
                '@Stateless' || CHR(13)||CHR(10) ||
                '@LocalBean' || CHR(13)||CHR(10) ||
                '@Interceptors(BusinessInterceptor.class)' || CHR(13)||CHR(10) ||
                ...
                '     */' || CHR(13)||CHR(10) ||
                '    public Ejb' || UN_NOMBRE_CLASEJAVA || '() {' || CHR(13)||CHR(10) ||
                '    }' || CHR(13)||CHR(10) ;
    ...
    LOOP
      ...
      MI_DEFINICION := FC_DEFINICION_JAVA(UN_OWNER, UN_PAQUETE, RS.EMPIEZA, MI_FINAL);      
      --Identificar el retorno de la función
      MI_RETORNO    := FC_RETORNO_JAVA(UN_OWNER, UN_PAQUETE, RS.EMPIEZA,MI_FINAL, MI_TIPO,0);           
      --Generar parametros de  la función  o procedimiento
      MI_PARAMETROS:= FC_PARAMETROS_JAVA(UN_OWNER, UN_PAQUETE, RS.EMPIEZA, MI_FINAL);
      ...
      MI_PAR_ORACLE :=FC_PARAMETROS_ORACLE(UN_OWNER, UN_PAQUETE, RS.EMPIEZA, MI_FINAL);
      MI_VA_TRY:=0;
      IF INSTR(MI_PAR_ORACLE,'SysmanFunciones.convertirAFechaCadena',1)>0 THEN
        MI_VA_TRY :=-1;
      END IF;

      IF MI_TIPO='F' THEN
        IF MI_RETORNO='boolean' THEN
          MI_DEFINICION := MI_DEFINICION || '         byte salida;' || CHR(13)||CHR(10);           
          IF MI_VA_TRY<>0 THEN
            MI_DEFINICION := MI_DEFINICION || '         try {' || CHR(13)||CHR(10);  
          END IF;
          MI_DEFINICION := MI_DEFINICION || '         String[] parametros ={' || MI_PAR_ORACLE || '};' || CHR(13)||CHR(10);   
          MI_DEFINICION := MI_DEFINICION || '         salida = (byte) AccionesImp.ejecutarFuncion(ConectorPool.ESQUEMA_SYSMAN, ';           
        ELSIF FC_RETORNO_JAVA(UN_OWNER, UN_PAQUETE, RS.EMPIEZA, MI_FINAL, MI_TIPO,-1) = 'clob' THEN
          MI_RETORNO := 'Clob';
          IF MI_VA_TRY<>0 THEN
            MI_DEFINICION := MI_DEFINICION || '         try {' || CHR(13)||CHR(10);  
          END IF;
          MI_DEFINICION := MI_DEFINICION || '         String[] parametros ={' || MI_PAR_ORACLE || '};' || CHR(13)||CHR(10);   
          MI_DEFINICION := MI_DEFINICION || '         return Acciones.clobToString((' || MI_RETORNO || ') AccionesImp.ejecutarFuncion(ConectorPool.ESQUEMA_SYSMAN, ';  
        ELSE
         IF MI_VA_TRY<>0 THEN
            MI_DEFINICION := MI_DEFINICION || '         try {' || CHR(13)||CHR(10);  
          END IF;
          MI_DEFINICION := MI_DEFINICION || '         String[] parametros ={' || MI_PAR_ORACLE || '};' || CHR(13)||CHR(10);   
          MI_DEFINICION := MI_DEFINICION || '         return (' || MI_RETORNO || ') AccionesImp.ejecutarFuncion(ConectorPool.ESQUEMA_SYSMAN, ';           
        END IF;
      ...
      --JP MI_DEFINICION := MI_DEFINICION || MI_PAR_ORACLE;
      MI_DEFINICION := MI_DEFINICION || 'SysmanFunciones.concatenar(parametros)';
      ...
      IF MI_TIPO='F' THEN
        ...
        ELSIF FC_TIPOS_JAVA(MI_RETORNO,-1) = 'double' THEN
          MI_DEFINICION := MI_DEFINICION || '                Types.DOUBLE';               
        ELSIF FC_TIPOS_JAVA(MI_RETORNO,-1) = 'BigDecimal' THEN
          MI_DEFINICION := MI_DEFINICION || '                Types.DECIMAL';               
        ELSIF FC_TIPOS_JAVA(MI_RETORNO,-1) = 'BigInteger' THEN
          MI_DEFINICION := MI_DEFINICION || '                Types.BIGINT';               
        ...
        ELSIF FC_TIPOS_JAVA(MI_RETORNO,-1) = 'boolean' THEN
          MI_DEFINICION := MI_DEFINICION || '                Types.TINYINT';
        ELSE
      ...
      IF MI_VA_TRY<>0 THEN
        MI_DEFINICION := MI_DEFINICION || '         }' || CHR(13)||CHR(10);
        MI_DEFINICION := MI_DEFINICION || '         catch (ParseException e) {' || CHR(13)||CHR(10);
        MI_DEFINICION := MI_DEFINICION || '             throw new SystemException(e);' || CHR(13)||CHR(10);
        MI_DEFINICION := MI_DEFINICION || '          }' || CHR(13)||CHR(10);  
      END IF;

      IF MI_TIPO='F' AND MI_RETORNO='boolean' THEN
        MI_DEFINICION := MI_DEFINICION || '                return salida == 0 ? false : true;' || CHR(13)||CHR(10);
      END IF;
    ...
    IF INSTR(MI_SALIDA, 'Date',1)>0 THEN
      MI_SALIDA:= REPLACE(MI_SALIDA,'EJB_ETAD_EJB','import java.util.Date;'  || CHR(13)||CHR(10) || 
                                                   'import java.text.ParseException;'  || CHR(13)||CHR(10) ||
                                                   'import com.sysman.util.SysmanFunciones;'   || CHR(13)||CHR(10) );
    ELSE
...
*/
  ...
  BEGIN  
    ...
    END LOOP RecorrerFuncionesProcedimiento;  
    IF INSTR(MI_SALIDA, 'Date',1)>0 THEN
      MI_SALIDA:= REPLACE(MI_SALIDA,'EJB_ETAD_EJB','import java.util.Date;'  || CHR(13)||CHR(10) || 'import java.text.ParseException;'  || CHR(13)||CHR(10) );
    ELSE
      MI_SALIDA:= REPLACE(MI_SALIDA,'EJB_ETAD_EJB','') ;
    END IF;
    RETURN MI_SALIDA || '}';
...
BEGIN
  ...
  IF SUBSTR(MI_ENTRADA,1,13) ='PCK_SUBTIPOS.' THEN
    ...
    ELSIF MI_SUBTIPO='TI_ENTERO' or MI_SUBTIPO='TI_MODULO' THEN
      MI_SALIDA := 'int';
       RETURN MI_SALIDA;
    ELSIF MI_SUBTIPO='TI_LONG' THEN
      MI_SALIDA := 'BigInteger';
      RETURN MI_SALIDA;
      ...
      SELECT  CASE WHEN DATA_TYPE='NUMBER' AND DATA_PRECISION =1 AND DATA_SCALE=0 
              ...
              ELSE CASE WHEN DATA_TYPE='NUMBER' AND DATA_PRECISION <9 AND DATA_SCALE=0 
                   ...
                   ELSE CASE WHEN DATA_TYPE='NUMBER' AND DATA_PRECISION >=9 AND DATA_PRECISION <=18 AND DATA_SCALE=0 
                        THEN 'long' 
                        ELSE CASE WHEN DATA_TYPE='NUMBER' AND DATA_PRECISION >=19 AND DATA_SCALE=0 
                            THEN 'BigInteger' 
  ...
  IF MI_ENTRADA='VARCHAR2' OR (MI_ENTRADA='CLOB' AND UN_CLOBAPARTE=0) THEN
    MI_SALIDA:='String';
  ELSIF MI_ENTRADA='CLOB' AND UN_CLOBAPARTE<>0 THEN
    MI_SALIDA:='clob';
  ELSIF MI_ENTRADA='DATE' OR MI_ENTRADA='TIMESTAMP' OR MI_ENTRADA='ZONE' THEN
    MI_SALIDA:='Date';
  ELSIF MI_ENTRADA='NUMBER' OR MI_ENTRADA='FLOAT' THEN
    MI_SALIDA:='BigDecimal';
...
BEGIN
  --Identificar el retorno de la función
      IF UN_TIPO ='F' THEN
        BEGIN
          ...
          WHERE REGEXP_LIKE(TRIM(TEXT), 'RETURN [^;]+$')
            ...
            AND TYPE  = 'PACKAGE BODY'
            AND LINE BETWEEN UN_LINEA_INI AND UN_LINEA_FIN;        
          MI_RETORNO := TRIM(REPLACE(MI_RETORNO,')',''));
      ...
      MI_RETORNO:= FC_TIPOS_JAVA( MI_RETORNO,UN_CLOBAPARTE);
      RETURN MI_RETORNO;
END FC_RETORNO_JAVA;
...
(
  ...
  MI_TIPOJAV   VARCHAR2(32000) DEFAULT '';
BEGIN
  --Generar parametros de  la función  o procedimiento
  MI_CAMPOTIPO:= FC_PARAMETROS(UN_OWNER, UN_PAQUETE, UN_LINEA_INI,UN_LINEA_FIN );
  MI_SALIDA:='                   ';
  IF MI_CAMPOTIPO.COUNT>0 THEN
    ...
    LOOP
      ...
      MI_TIPOJAV := FC_TIPOS_JAVA(MI_CAMPOTIPO(i).TIPO,0);
      IF MI_TIPOJAV = 'Date' THEN
        MI_SALIDA := MI_SALIDA || 'TO_DATE(''' || CHR(34) || ' , SysmanFunciones.convertirAFechaCadena('  || MI_PARA ||  ') , ' || CHR(34)  || ''',''DD/MM/YYYY''), ' ;
      ELSIF MI_TIPOJAV = 'boolean' THEN
        MI_SALIDA := MI_SALIDA ||  CHR(34) || ' , ('  || MI_PARA ||  '?' || CHR(34) || '-1'  || CHR(34) || ':' || CHR(34) || '0' || CHR(34) || ')  , '  || CHR(34) ||  ', ';
      ELSIF MI_TIPOJAV = 'int' THEN
        MI_SALIDA := MI_SALIDA  ||  CHR(34) || ' , Integer.toString(' || MI_PARA ||  ') , '  || CHR(34) ||  ', ';
      ELSIF MI_TIPOJAV = 'BigInteger' OR MI_TIPOJAV = 'BigDecimal' THEN
        MI_SALIDA := MI_SALIDA ||  CHR(34) || ' ,' ||  MI_PARA ||  '.toString() ,'  || CHR(34) ||  ', ';
      ELSIF MI_TIPOJAV = 'double' THEN
        MI_SALIDA := MI_SALIDA ||  CHR(34) || ' , Double.toString(' || MI_PARA ||  ')   ,'  || CHR(34) ||  ', ';
      ELSIF MI_TIPOJAV = 'long' THEN
        MI_SALIDA := MI_SALIDA ||  CHR(34) || ' , Long.toString(' || MI_PARA ||  ')   ,'  || CHR(34) ||  ', ';
      ELSE
        MI_SALIDA := MI_SALIDA || MI_TIPO || CHR(34) || ' , '  || MI_PARA ||  ' , ' || CHR(34)  || MI_TIPO ||  ', ';
        --MI_SALIDA := MI_SALIDA || CHR(34);
      END IF;
      MI_SALIDA := MI_SALIDA || CHR(34) || CHR(13)||CHR(10) || '                 , ' ;            
      --MI_SALIDA := MI_SALIDA ||  CHR(13)||CHR(10) || '                 , ' ;            
    END LOOP;
    MI_SALIDA := SUBSTR(MI_SALIDA,1,INSTR(MI_SALIDA, ', ' ||CHR(34),-1 )-1) || CHR(34);
  END IF;
  IF LENGTH(MI_SALIDA)>3 THEN
    IF SUBSTR(MI_SALIDA, -3)= ',' || CHR(34) || CHR(34)  THEN
      MI_SALIDA := SUBSTR(MI_SALIDA,1, LENGTH(MI_SALIDA)-3);
    END IF;
  END IF;
  RETURN MI_SALIDA || CHR(13)||CHR(10);
END FC_PARAMETROS_ORACLE;
